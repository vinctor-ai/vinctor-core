...
from vinctor_core.audit import EVENT_KEY_CREATED, EVENT_KEY_REVOKED, EVENT_KEY_ROTATED
...
def create_pep_local_key(
    ...
    *,
    ...
    """Mint an ADDITIONAL PEP (resource-server) key, revoking nothing (PKA-180).

    A PEP authenticates to ``POST /v1/enforce/delegated`` with an ``X-PEP-Key``
    header, and nothing shipped could mint that secret: ``rotate pep`` is the
    wrong operation for standing up a new PEP, because it revokes the key the
    already-deployed PEP is still using. Creation and its ``key_created``
    control event commit as ONE transaction, exactly as rotation and revocation
    do — a key can never exist without an audit row naming who minted it.
    """
    # Fail closed BEFORE any write: the creation and its control audit event are
    # one transaction only if the auditor writes through the repository's
    # connection.
    control_auditor.require_bound_to(getattr(repository, "_conn", None))
    with repository.transaction():
        ...
        control_auditor.record(
            event_type=EVENT_KEY_CREATED,
            workspace_id=workspace_id,
            agent_id=pep_id,
            action="create_pep_key",
            resource=f"key/{created.record.key_type}/{created.record.key_id}",
            reason="status=active",
            now=now,
            enforcing_principal=enforcing_principal,
        )
    # Return the plaintext only after the creation has committed.
    return created
