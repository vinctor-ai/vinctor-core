...
@runtime_checkable
class BestEffortAuditWriter(Protocol):
...
def record_rejection(audit_writer: AuditWriter, event: AuditEvent) -> bool:
    ...
    """
    try:
        if isinstance(audit_writer, BestEffortAuditWriter):
            return audit_writer.write_best_effort(event)
