...
def _in_transaction(context: SQLiteRequestContext) -> bool:
    # A closed/broken connection raises rather than answering; treat anything
    # that cannot be inspected as dirty so it is retired, not reused.
    try:
        return bool(context.connection.in_transaction)
    except Exception:
        return True


def _rollback(context: SQLiteRequestContext) -> bool:
    """Roll the connection back; True only if it ends up clean."""
    try:
        context.connection.rollback()
    except Exception:
        return False
    return not _in_transaction(context)
...
class SQLiteServicePool:
    ...
    def request_scope(self) -> Iterator[None]:
        ...
        try:
            ...
            self._reclaim(context)

    def _reclaim(self, context: SQLiteRequestContext) -> None:
        """Return a context to the pool, or retire it if it is still dirty.

        A request whose COMMIT failed leaves the transaction OPEN — SQLite does
        not roll back on a failed commit. Handing that connection to the next
        request would make its write scopes take the "already in a transaction"
        nesting path and join an orphaned transaction that is never committed:
        enforce would keep answering `permit` while no audit row ever became
        durable, for as long as the process lived (PKA-224).

        So the pool rolls back on the way out, and a connection that is STILL in
        a transaction after that is not reusable at all — it is retired and
        replaced rather than returned.
        """
        if _in_transaction(context) and not _rollback(context):
    ...
    def _retire(self, context: SQLiteRequestContext) -> None:
        """Drop a context from the pool and close it, then rebuild to size."""
        with self._state_lock:
            if context.closed or context not in self._contexts:
                return
        ...
        with suppress(Exception):
            context.connection.close()
        # Replenishment must not raise out of a request's finally block and
        # replace the exception the request was already unwinding with.
        with suppress(Exception):
            self._replenish_once()
