...
def run_vinctor(
    ...
    *,
    ...
    try:
        ...
        AuditSinkConfigurationError,
        PostgresDriverUnavailable,
        SchemaVersionError,
    ) as error:
        # Fail-closed startup gates are operator configuration errors, so surface
        # the actionable reason without an internal traceback.
...
_EXPLICIT_FLAGS_DEST = "explicit_flags"


class _RecordExplicitFlag(argparse.Action):
    """Store the value and remember that it came from the command line.

    Global flags fall back to environment variables, so the parsed value alone
    cannot tell an operator's typed ``--endpoint …`` apart from an exported
    ``VINCTOR_ENDPOINT``. A command that must refuse a flag rather than discard
    it needs that distinction: an exported value is ambient configuration, a
    typed flag is an instruction.
    """
    ...
    def __call__(
        ...
        explicit = getattr(namespace, _EXPLICIT_FLAGS_DEST, None)
        if explicit is None:
            explicit = set()
            setattr(namespace, _EXPLICIT_FLAGS_DEST, explicit)
        explicit.add(option_string or self.option_strings[0])
...
def _add_global_flags(parser: argparse.ArgumentParser, *, defaults: bool) -> None:
    ...
    def add(option_strings: list[str], *, record: bool = True, **kwargs: object) -> None:
        if not defaults:
            existing = {
                option
                for action in parser._actions
        ...
        if record:
            kwargs["action"] = _RecordExplicitFlag
...
def _add_local_start_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--boundary-name", default="claude-code-local")
...
def _add_operator_commands(roles: argparse._SubParsersAction) -> None:
    ...
    audit_list.add_argument(
        "--reason",
        help="Filter by deny reason or rejection reason_code "
        "(e.g. action_denied, boundary_required, pop_required).",
    )
    ...
    idempotency = resources.add_parser(
        "idempotency",
        help="Inspect and advance encrypted idempotency key lifecycle barriers.",
        description="Inspect non-secret idempotency key state or advance one explicit "
        "historical version through write-disable, drain, and retirement.",
    )
    lifecycle_commands = idempotency.add_subparsers(
        dest="idempotency_command",
        required=True,
    )
    lifecycle_commands.add_parser(
        "status",
        help="Show bounded, non-secret idempotency key lifecycle state.",
    )
    write_disable = lifecycle_commands.add_parser(
        "write-disable",
        help="Disable new reservations for one explicit historical version.",
    )
    write_disable.add_argument("--version")
    write_disable.add_argument("--reason", choices=("rotation",))
    drain_complete = lifecycle_commands.add_parser(
        "drain-complete",
        help="Record the externally confirmed old-writer drain barrier.",
    )
    drain_complete.add_argument("--version")
    drain_complete.add_argument(
        "--confirm-no-active-writers",
        action="store_true",
    )
    retire = lifecycle_commands.add_parser(
        "retire",
        help="Retire one drained version after the DB-time removal window.",
    )
    ...
    keys = resources.add_parser(
        "keys",
        help="List, create, revoke, and rotate API keys.",
        description="Manage API keys: list, create, revoke, and rotate "
        "workspace/agent/PEP keys.",
    )
    ...
    keys_create = keys_commands.add_parser(
        "create",
        help="Create a new PEP key without revoking any existing key.",
        description="Create a new key, leaving existing keys active. The raw key is "
        "printed once and cannot be recovered later. Use `rotate` instead when you "
        "want the previous key revoked.",
    )
    create_targets = keys_create.add_subparsers(dest="create_target", required=True)
    create_pep = create_targets.add_parser(
        "pep",
        help="Create a PEP (resource-server) key.",
        description="Create a key for a PEP (resource server) so it can authenticate "
        "to POST /v1/enforce/delegated with the X-PEP-Key header. Existing PEP keys "
        "stay active.",
    )
    create_pep.add_argument("--pep-id", required=True)
...
def _add_demo_commands(roles: argparse._SubParsersAction) -> None:
    parser = roles.add_parser(
        ...
        "showcase, `service` is the fuller end-to-end walkthrough. None of them "
        "contacts a configured deployment; they refuse connection flags such as "
        "--endpoint rather than discard them.",
    )
    commands = parser.add_subparsers(dest="demo_command", required=True)
    commands.add_parser(
        "check",
        help="Smoke test of the packaged build (throwaway service, not your deployment).",
        description="Smoke test the packaged build: request -> auto-approve -> enforce "
        "(must permit) -> deny probe (must be denied) -> audit, all against a "
        "throwaway local service this command starts and destroys. It does NOT "
        "check a deployment: use `vinctor agent enforce` for that.",
    )
    commands.add_parser(
        "block",
...
def _warn_unrecognized_env() -> None:
    """PKA-168: say so when a ``VINCTOR_*`` variable configures nothing here.

    Called from the two commands that START a server, because that is where an
    operator's configuration takes effect and the moment they are watching. It
    is a warning, never a refusal: an upgrade that crashes on a leftover
    variable would be a worse failure than the silent no-op being fixed, and
    this process cannot know what else shares its environment.

    stderr, so it cannot corrupt the `--json` bodies these commands emit on
    stdout, and so it survives a caller piping stdout to a file.
    """
...
def _operator(args: argparse.Namespace, *, stdout: TextIO) -> None:
    ...
    if resource == "idempotency":
        _operator_idempotency(args, stdout=stdout)
        return
    if resource == "service":
...
def _operator_audit(args: argparse.Namespace, *, stdout: TextIO) -> None:
    ...
    if args.audit_command == "verify":
        ...
        ok = v.ok
        failure = "audit chain verification failed"
        ...
        if ok and args.against_anchor is not None:
            ...
            result["anchor"] = {
                ...
                "head_seq": av.head_seq,
                "divergence_seq": av.divergence_seq,
                "divergence_kind": av.divergence_kind,
            }
            ok = ok and av.ok
            if av.divergence_kind == "uncovered":
                failure = (
                    "audit chain verification failed: the anchor covers only "
                    f"through seq {av.covered_max_seq} but the chain head is "
                    f"{av.head_seq}; seq {av.divergence_seq}-{av.head_seq} are "
                    "unanchored and cannot be verified"
                )
        # `ok` is the WHOLE verdict — chain, expected head, AND anchor. A
        # consumer that parses `.ok` must never read a chain the anchor rejects
        # as healthy, so the emitted body carries the combined result rather
        # than the chain-only one.
        result["ok"] = ok
        _emit(args, result, "audit chain OK" if ok else "audit chain BROKEN", stdout=stdout)
        if not ok:
            raise CliError(failure, code=EXIT_DENIED, quiet_json=True)
        return
...
def _operator_storage(args: argparse.Namespace, *, stdout: TextIO) -> None:
    ...
    if command == "reset":
        ...
        result = reset_sqlite(_required_db(args.db))
        body = {
            "db_path": str(result.db_path),
            "schema_versions": list(result.schema_versions),
            "reset": True,
    ...
    if command == "restore":
        ...
        try:
            result = restore_sqlite(_required_db(args.db), args.input)
        ...
        return
    if command == "migrate":
        result = migrate_sqlite(_required_db(args.db))
        body = {
            "db_path": str(result.db_path),
            "schema_versions": list(result.schema_versions),
        }
...
def _operator_idempotency(args: argparse.Namespace, *, stdout: TextIO) -> None:
    ...
    try:
        ...
        controller = IdempotencyLifecycleController.from_config(config)
    except (OSError, ValueError, IdempotencyLifecycleUnavailable):
        raise CliError("idempotency_unavailable", code=EXIT_SERVICE) from None
    try:
        command = args.idempotency_command
        if command == "status":
            ...
            body = {
                ...
                "versions": [asdict(status) for status in statuses],
            }
            text = "\n".join(
                " ".join(
                    (
                        f"version={status.version}",
                        f"commitment_id={status.commitment_identifier}",
                        f"slots={status.reserved_encryption_slots}",
                        f"write_disabled={status.write_disabled_epoch or '-'}",
                        f"drain_completed={status.drain_completed_epoch or '-'}",
                        f"retired={status.retired_epoch or '-'}",
                    )
                )
            ...
            _emit(args, body, text, stdout=stdout)
            return
        version = _required(args.version, "version")
        if command == "write-disable":
            reason = _required(args.reason, "reason")
            controller.write_disable(version=version, reason=reason)
            _emit(
                args,
                {"ok": True, "operation": command, "version": version},
                f"write-disabled version={version}",
                stdout=stdout,
            )
            return
        if command == "drain-complete":
            controller.complete_drain(
                version=version,
                confirm_no_active_writers=args.confirm_no_active_writers,
            )
            _emit(
                args,
                {"ok": True, "operation": command, "version": version},
                f"drain-complete version={version}",
                stdout=stdout,
            )
            return
        if command == "retire":
            controller.retire(
                IdempotencyRetirementRequest(
                    version=version,
                    confirm_removal_window=args.confirm_removal_window,
                )
            )
            _emit(
                args,
                {"ok": True, "operation": command, "version": version},
                f"retired version={version}",
                stdout=stdout,
            )
            return
        raise CliError("unknown idempotency command")
    except IdempotencyLifecycleRejected as error:
        raise CliError(str(error)) from None
    except (IdempotencyLifecycleActiveWriters, IdempotencyLifecycleUnavailable) as error:
        raise CliError(str(error), code=EXIT_SERVICE) from None
    finally:
        controller.close()
...
def _operator_keys(args: argparse.Namespace, *, stdout: TextIO) -> None:
    ...
    if command == "list":
        ...
        return
    if command == "create":
        if args.create_target != "pep":
            raise CliError(f"unknown create target: {args.create_target}")
        created = create_pep_local_key(
            repository,
            workspace_id=args.workspace_id,
            pep_id=args.pep_id,
            now=now,
        ...
        text = "\n".join(
            [
                f"created {record.key_type} key key_id={record.key_id} "
                f"pep_id={record.agent_id}",
                f"raw_key={created.raw_key}",
...
# Every `demo` subcommand runs against a throwaway service it starts itself, so
# every flag that names or authenticates to a deployment is meaningless to it.
# Accepting one and silently discarding it is how `demo check` came to report
# `ok: true` against a dead endpoint, a wrong key and a nonexistent grant
# (PKA-214). Honouring them instead is not an option: the check writes an
# auto-approval rule, and a demo must never install standing policy in
# somebody's real authorization service.
_DEMO_REFUSED_FLAGS = (
    "--endpoint",
    "--workspace-key",
    "--auditor-key",
...
def _reject_demo_connection_flags(args: argparse.Namespace) -> None:
    supplied = getattr(args, _EXPLICIT_FLAGS_DEST, None) or set()
    refused = [flag for flag in _DEMO_REFUSED_FLAGS if flag in supplied]
    if not refused:
        return
    raise CliError(
        "demo commands never contact a configured deployment: they run against a "
        "throwaway local service they start and destroy. Refusing to discard "
        + ", ".join(refused)
        + ". Drop the flag to run the demo, or exercise your real deployment with "
        "`vinctor agent enforce --endpoint ... --agent-key ... --grant-ref ... "
        "--action ... --resource ...`."
    )


def _demo(args: argparse.Namespace, *, stdout: TextIO) -> None:
    _reject_demo_connection_flags(args)
    ...
    _demo_check(args, stdout=stdout)


# The scopes the check grants itself. Widening or narrowing these is how a test
# seeds a broken deployment: too broad and the deny probe is permitted, too
# narrow and the granted action is denied. Both must fail the check.
_DEMO_CHECK_GRANT_SCOPES = ("execute:ci/test",)
_DEMO_DENY_PROBE_PREFIX = "vinctor/demo/deny-probe"


def _demo_check(args: argparse.Namespace, *, stdout: TextIO) -> None:
    with TemporaryDirectory() as temp_dir:
        handle = prepare_local_service(
            LocalLaunchConfig(
                ...
                scopes=_DEMO_CHECK_GRANT_SCOPES,
        ...
        try:
            ...
            agent_headers = _agent_headers(handle, boundary_id)
            workspace_headers = {"X-Workspace-Key": handle.workspace_key}
            _demo_check_post(
                handle.endpoint,
                "/v1/auto-approval-rules",
                ...
                body={
                    ...
                    "allowed_scopes": list(_DEMO_CHECK_GRANT_SCOPES),
                    "max_ttl_seconds": 1800,
                },
            )
            created = _demo_check_post(
                handle.endpoint,
                "/v1/grant-requests",
                headers={"X-Agent-Key": handle.agent_key},
                body={
                    "scopes": list(_DEMO_CHECK_GRANT_SCOPES),
                    "ttl_seconds": 900,
                    "reason": "demo check",
                },
            )
            approved = _demo_check_post(
                handle.endpoint,
            ...
            # Positive control: the granted action must come back permitted. A
            # 2xx that is not a verifiable permit is a failure, not a pass.
            permitted = _demo_check_post(
                handle.endpoint,
                "/v1/enforce",
                headers=agent_headers,
                body={"grant_ref": grant_ref, "action": "execute", "resource": "ci/test"},
            )
            if permitted.get("decision") != "permit":
                raise CliError(
                    "self-check failed: the granted action execute:ci/test came back "
                    f"{permitted.get('decision') or 'undecided'!r}, not permitted",
                    code=EXIT_SERVICE,
                )

            # Negative control: a positive control alone proves nothing -- a
            # service that rubber-stamps every request would pass it. The probe
            # resource is fresh per run so it cannot have been granted.
            probe_resource = f"{_DEMO_DENY_PROBE_PREFIX}/{uuid4()}"
            ...
            probe_reason = _demo_check_deny_verdict(probe_status, probe_body)

            recorded = {event.event_type for event in handle.service.audit_events}
            missing = {"action_permitted", "action_denied"} - recorded
            if missing:
                raise CliError(
                    "self-check failed: both decisions must reach the audit log, but "
                    f"{', '.join(sorted(missing))} was never written",
                    code=EXIT_SERVICE,
                )
            ...
            body = {
                "ok": True,
                "target": "throwaway",
                "throwaway_endpoint": handle.endpoint,
                "request_id": created["request_id"],
                "grant_ref": grant_ref,
                "decision": permitted["decision"],
                "deny_probe_resource": probe_resource,
                "deny_probe_decision": probe_body["decision"],
                "deny_probe_reason": probe_reason,
                "audit_event_count": len(handle.service.audit_events),
            }
            _emit(args, body, _demo_check_text(body), stdout=stdout)
        finally:
...
def _demo_check_post(
    ...
    *,
    ...
    status, parsed = _request_json(endpoint, "POST", path, headers=headers, body=body)
    _raise_for_status(status, parsed)
    return parsed


def _demo_check_deny_verdict(status: int, body: dict[str, object]) -> str:
    """Return the denial reason, or fail the self-check.

    The absence of a verdict is not a denial. A permit means the service is not
    enforcing the grant boundary at all; an auth failure, a server error, or any
    response carrying no `deny` decision means the probe never produced a
    verdict. Neither may be read as "nothing got through".
    """
    decision = body.get("decision")
    if status == 403 and decision == "deny":
        return str(body.get("reason") or "action_denied")
    if decision == "permit":
        raise CliError(
            "self-check failed: the deny probe was PERMITTED -- the service is not "
            "enforcing the grant boundary",
            code=EXIT_SERVICE,
        )
    detail = str(body.get("error") or body.get("reason") or "no decision")
    raise CliError(
        f"self-check failed: the deny probe returned no verdict (HTTP {status}: "
        f"{detail}) -- the absence of a denial is not a denial",
        code=EXIT_SERVICE,
    )


def _demo_check_text(body: dict[str, object]) -> str:
    verdict = _demo_verdict_label
    return "\n".join(
        [
            "▸ demo check passed against a throwaway local service @ "
            f"{body['throwaway_endpoint']}",
            f"  {verdict(body['decision'])}   execute:ci/test   granted action",
            f"  {verdict(body['deny_probe_decision'])}    delete:{body['deny_probe_resource']}",
            f"          negative control -- {body['deny_probe_reason']}",
            f"  {body['audit_event_count']} audit records, including both decisions.",
            "",
            "  This proves the packaged Vinctor build enforces a grant on this machine.",
            "  It does not check your deployment: demo commands never contact a",
            "  configured endpoint, key or grant. To exercise a real deployment run",
            "  `vinctor agent enforce --endpoint ... --grant-ref ... --action ... "
            "--resource ...`.",
        ]
    )
...
def _demo_service_text(body: dict[str, object]) -> str:
    ...
    return "\n".join(
        [
            f"▸ Vinctor demo service running @ {body['throwaway_endpoint']} (throwaway)",
...
def _demo_block_text(body: dict[str, object]) -> str:
    ...
    lines.append(
        "  Vinctor authorizes configured, mediated calls that reach its boundary; "
        "it is not a sandbox."
    )
    return "\n".join(lines)
...
# Almost every `operator` resource talks to SQLite directly; only grant
# requests, auto-approval rules, and grant revocation have remote routes so far
# (ADR 0019 — the remote operator surface is an authenticated HTTP API being
# added route by route, and the direct-DB path remains for the single-box case).
# An operator meeting that split control plane for the first time reads a bare
# "db is required" as a typo, so name the flag and say why this one is local.
_DB_REQUIRED_MESSAGE = (
    "--db is required: this command reads the local SQLite database directly "
    "because the service exposes no remote operator route for it (only "
    "`operator requests`, `operator rules`, and `operator grants revoke` work "
    "over --endpoint). Pass --db /path/to/vinctor.db or set VINCTOR_DB."
)


def _required_db(db_path: Path | None) -> Path:
    if db_path is None or str(db_path) == "":
        raise CliError(_DB_REQUIRED_MESSAGE)
    return Path(db_path)


def _sqlite_service(db_path: Path | None) -> SQLiteV1Service:
    conn = connect_sqlite(_required_db(db_path))
...
def _audit_event_matches(event: object, args: argparse.Namespace) -> bool:
    ...
    if args.boundary_id and event.boundary_id != args.boundary_id:
        return False
    # Match either field: rejection rows carry the cause in `reason_code`, but a
    # decision deny carries it in `reason` (and no reason_code at all). The list
    # output prints `reason=`, so filtering only on reason_code silently returned
    # nothing for the code the operator had just read off the screen (PKA-254).
    if args.reason and args.reason not in (event.reason_code, event.reason):
        return False
    if args.request_id:
