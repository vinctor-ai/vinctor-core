...
class V1TokenMintHttpResult:
    response: V1HttpResponse
    authoritative_result: SubjectTokenMintResult = dataclass_field(repr=False)
...
def handle_v1_tokens_http(
    *,
    ...
    parsed = parse_v1_token_mint_body(body, max_ttl=max_ttl)
    if isinstance(parsed, V1HttpResponse):
        return parsed

    return mint_v1_token_http_result(
        parsed,
        identity=identity,
        service=service,
        now=now,
...
def mint_v1_token_http_result(
    parsed: ParsedTokenMintBody,
    *,
    identity: AgentIdentity,
    service: V1TokenService,
    now: datetime,
) -> V1TokenMintHttpResult:
    ...
    if result.status != "minted":
        return V1TokenMintHttpResult(
            response=_error(403, "forbidden", "subject token could not be issued"),
    ...
    return V1TokenMintHttpResult(
        response=V1HttpResponse(status_code=201, body=body),
        authoritative_result=result,
    )


def parse_v1_token_mint_body(
    body: object,
    *,
    max_ttl: int,
) -> ParsedTokenMintBody | V1HttpResponse:
    ...
    if (bound_action is None) != (bound_resource is None):
        return _error(400, "invalid_request", "action and resource must be set together")
