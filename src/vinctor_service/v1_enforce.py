...
from vinctor_core.audit import (
    ...
    REASON_POP_PROOF_INVALID,
    REASON_POP_REQUIRED,
    REASON_SUBJECT_TOKEN_EXPIRED,
    REASON_SUBJECT_TOKEN_MISMATCH,
    REASON_SUBJECT_TOKEN_NOT_FOUND,
    REASON_SUBJECT_TOKEN_REQUIRED,
    REASON_SUBJECT_TOKEN_REVOKED,
    AuditEventInput,
    agent_facing_reason,
    build_audit_event,
    ...
    *,
    ...
    if request.subject_token is not None:
        ...
        if token is None:
            _record_rejection(
                audit_writer,
                reason_code=REASON_SUBJECT_TOKEN_NOT_FOUND,
                workspace_id=trusted_ws,
                agent_id=request.agent_id,
                action=request.action,
                resource=request.resource,
        ...
        # SAME generic caller-facing result as any other invalid token (never
        # reveal whether it was revoked vs expired). Only the operator-only audit
        # reason_code separates them — revoking is a deliberate operator act, so
        # "this is working as intended" and "the agent must re-mint" are
        # different answers to give the operator reading the trail.
        if token.revoked_at is not None:
            _record_rejection(
                audit_writer,
                reason_code=REASON_SUBJECT_TOKEN_REVOKED,
                workspace_id=trusted_ws,
                agent_id=request.agent_id,
                action=request.action,
                resource=request.resource,
            ...
            return _pre_audit_error(403, "forbidden", "subject token is not valid")
        # Expiry is checked on its own (ahead of the identity tuple below) purely
        # so the operator audit can say "expired" rather than the generic
        # mismatch. The caller-facing result is byte-identical either way, so the
        # reordering is not observable outside the audit trail.
        if token.expires_at <= now:
            _record_rejection(
                audit_writer,
                reason_code=REASON_SUBJECT_TOKEN_EXPIRED,
                workspace_id=trusted_ws,
                agent_id=request.agent_id,
                action=request.action,
                resource=request.resource,
            ...
            return _pre_audit_error(403, "forbidden", "subject token is not valid")
        # audience (target pep_id) + identity tuple (body and grant) +
        ...
        if (
            ...
            _record_rejection(
                audit_writer,
                reason_code=REASON_SUBJECT_TOKEN_MISMATCH,
                workspace_id=trusted_ws,
                agent_id=request.agent_id,
                action=request.action,
                resource=request.resource,
        ...
        if token.pop_secret is not None:
            ...
            if not ok:
                _record_rejection(
                    audit_writer,
                    reason_code=REASON_POP_PROOF_INVALID,
                    workspace_id=trusted_ws,
                    agent_id=request.agent_id,
                    action=request.action,
                    resource=request.resource,
