...
class PostgresAgentEnforcementSettingsRepository:
    ...
    def get_require_boundary(self, *, workspace_id: str, agent_id: str) -> bool:
        # Effective mandate, resolved as the enforce path resolves it. See the
        # SQLite implementation for why reading only the agent row is unsafe.
...
class PostgresAuditWriter:
    ...
    def verify_against_anchor(self, records: list[AnchorRecord]) -> AnchorVerification:
        # Coverage that stops below the chain head is a FAILURE, not a partial
        # pass — see SQLiteAuditWriter.verify_against_anchor for why. MUST stay
        # behaviourally identical to the SQLite backend.
