...
def _sqlite_apply_transaction(conn: sqlite3.Connection) -> Iterator[None]:
    ...
    with conn_txn_lock(conn), conn.atomic_write_deferral():
        ...
        try:
            yield
            # The commit MUST be inside the try (sqlite._write_scope is the
            # reference shape). SQLite leaves the transaction OPEN when COMMIT
            # returns SQLITE_BUSY, so a commit outside the try propagates the
            # error with the connection still dirty — and every later scope on
            # it takes the "already in a transaction" nesting path, joining an
            # orphaned transaction that is never committed (PKA-224).
            conn.commit()
    ...
    *,
    ...
    for entry in _required_list(document, "agent_bounds"):
        ...
        validate_issuable_scope_bounds(
            tuple(parsed["scopes"]), max_ttl_seconds=parsed["max_ttl_seconds"]
        )
    ...
    # Every entry validated; apply all writes. A bounds entry silent on the
    # max TTL passes None WITHOUT clear_max_ttl: the repository preserves the
    # stored cap, so re-applying a policy never widens issuable authority
    # (PKA-102). Only an explicit null in the document clears the cap.
    for parsed in parsed_bounds:
        service.set_agent_issuable_scope_bounds(
            ...
            max_ttl_seconds=parsed["max_ttl_seconds"],
            clear_max_ttl=parsed["max_ttl_declared"] and parsed["max_ttl_seconds"] is None,
            now=now,
            enforcing_principal=applied_by,
        )
...
def export_policy_document(
    *,
    ...
    bounds_entries: list[dict[str, object]] = []
    for agent_id, scopes in service.scope_bounds_repository.list_bounds_for_workspace(
        workspace_id
    ):
        entry: dict[str, object] = {"agent_id": agent_id, "scopes": list(scopes)}
        issuable = service.scope_bounds_repository.get_bounds_with_max_ttl(
            workspace_id=workspace_id, agent_id=agent_id
        )
        # The cap rides in the export so export->edit->apply is lossless
        # (PKA-102), emitted as the same `max_ttl` duration string the apply
        # grammar reads. An uncapped agent OMITS the key rather than writing
        # an explicit null: absent means preserve on apply, so re-applying an
        # export can never silently clear a cap set in the meantime.
        if issuable is not None and issuable.max_ttl_seconds is not None:
            entry["max_ttl"] = f"{issuable.max_ttl_seconds}s"
        bounds_entries.append(entry)
    document: dict[str, object] = {
        "version": 1,
        "workspace_id": workspace_id,
        "agent_bounds": bounds_entries,
        "auto_approval_rules": [
            {
...
def _parse_bounds_entry(entry: object) -> dict[str, Any]:
    ...
    _reject_extra(
        mapping, {"agent_id", "scopes", "max_ttl", "max_ttl_seconds"}, "agent_bounds entry"
    )
    agent_id = _required_string(mapping, "agent_id")
    scopes = _required_string_list(mapping, "scopes")
    max_ttl_declared, max_ttl_seconds = _bounds_max_ttl(mapping)
    return {
        "agent_id": agent_id,
        "scopes": scopes,
        "max_ttl_declared": max_ttl_declared,
        "max_ttl_seconds": max_ttl_seconds,
    }


def _bounds_max_ttl(mapping: dict[str, object]) -> tuple[bool, int | None]:
    # Three-way grammar (PKA-102): an ABSENT key preserves any stored cap, an
    # explicit null CLEARS it, and a value SETS it — so a policy silent on the
    # cap can never silently widen issuable authority.
    if "max_ttl" in mapping and "max_ttl_seconds" in mapping:
        raise ValueError("agent_bounds entry must not set both max_ttl and max_ttl_seconds")
    if "max_ttl_seconds" in mapping:
        ...
        return True, value
    if "max_ttl" in mapping:
        raw = mapping["max_ttl"]
        if raw is None:
            return True, None
        if not isinstance(raw, str) or raw == "":
            raise ValueError(f"invalid max_ttl duration: {raw}")
        return True, _duration_string_seconds(raw)
    return False, None
...
def _duration_seconds(mapping: dict[str, object]) -> int:
    ...
    return _duration_string_seconds(raw)


def _duration_string_seconds(raw: str) -> int:
    suffix = raw[-1]
    number = raw[:-1]
