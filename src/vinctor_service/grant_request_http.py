...
GrantRequestDecisionAction: TypeAlias = Literal["approve", "reject"]
...
@dataclass(frozen=True, slots=True)
class ParsedGrantRequestDecisionBody:
    decision_reason: str | None


@dataclass(frozen=True, slots=True)
class ParsedGrantRequestAutoApproveBody:
    pass
        ...
        if (
            ...
            if parts[1] == "auto-approve":
                ...
                if isinstance(parsed_auto_approve, V1HttpResponse):
                    return parsed_auto_approve
                return auto_approve_v1_grant_request_response(
                    service=service,
                    request_id=parts[0],
            ...
            return decide_v1_grant_request_response(
                service=service,
                request_id=parts[0],
                action=parts[1],
                identity=workspace_identity,
...
def auto_approve_v1_grant_request_response(
    *,
    service: GrantRequestService,
    request_id: str,
...
def decide_v1_grant_request_response(
    *,
    service: GrantRequestService,
    request_id: str,
    action: GrantRequestDecisionAction,
    identity: WorkspaceIdentity,
    parsed: ParsedGrantRequestDecisionBody,
    now: datetime,
) -> V1HttpResponse:
    decided_by = f"workspace:{identity.workspace_id}"
    if action == "approve":
...
def parse_v1_grant_request_decision_body(
    body: object,
) -> ParsedGrantRequestDecisionBody | V1HttpResponse:
    if body is None:
        return ParsedGrantRequestDecisionBody(decision_reason=None)
    if not isinstance(body, dict):
        return _error(400, "invalid_request", "request body must be a JSON object")
    ...
    return ParsedGrantRequestDecisionBody(decision_reason=value)


def parse_v1_grant_request_auto_approve_body(
    body: object,
) -> ParsedGrantRequestAutoApproveBody | V1HttpResponse:
    if body is not None:
        return _error(
            400,
            "invalid_request",
    ...
    return ParsedGrantRequestAutoApproveBody()
...
def _agent_can_view_request(identity: AgentIdentity, request: GrantRequest) -> bool:
    return request.workspace_id == identity.workspace_id and identity.agent_id in {
