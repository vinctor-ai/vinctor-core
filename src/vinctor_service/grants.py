...
class InMemoryAgentIssuableScopeBoundsRepository:
    ...
    def set_bounds(
        ...
        *,
        ...
        # Partial update: an undeclared max TTL preserves the stored cap
        # (PKA-102); removing a cap is the explicit clear_max_ttl act.
        if max_ttl_seconds is None and not clear_max_ttl:
            existing = self._bounds.get((workspace_id, agent_id))
            if existing is not None:
                max_ttl_seconds = existing.max_ttl_seconds
    ...
    *,
    ...
    if issuable is None:
        ...
        return GrantIssueResult(
            ...
            detail=(
                f"no issuable scope bounds are configured for agent '{request.target_agent_id}'"
            ),
        )
...
def validate_clear_max_ttl(*, max_ttl_seconds: int | None, clear_max_ttl: bool) -> None:
    if clear_max_ttl and max_ttl_seconds is not None:
        raise ValueError("clear_max_ttl requires max_ttl_seconds to be None")


def set_bounds_audit_reason(*, max_ttl_seconds: int | None, clear_max_ttl: bool) -> str:
    # The audit trail must say what actually happened to the cap: an
    # undeclared max TTL PRESERVES the stored value (PKA-102), so recording
    # it as "none" would misreport a still-binding cap.
    if clear_max_ttl:
        return "max_ttl_seconds=cleared"
    if max_ttl_seconds is None:
        return "max_ttl_seconds=preserved"
    return f"max_ttl_seconds={max_ttl_seconds}"
