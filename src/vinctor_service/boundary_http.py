...
class ParsedBoundaryCreateBody:
    name: str
    runtime: str
    boundary_type: str
    mode: BoundaryMode
...
def _create_boundary(
    ...
    *,
    ...
    parsed = parse_v1_boundary_create_body(body)
    if isinstance(parsed, V1HttpResponse):
        return parsed

    return create_v1_boundary_response(
        parsed,
        identity=identity,
        service=service,
        now=now,
...
def create_v1_boundary_response(
    parsed: ParsedBoundaryCreateBody,
    *,
    identity: WorkspaceIdentity,
    service: BoundaryAdminService,
    now: datetime,
...
def parse_v1_boundary_create_body(
    body: object,
) -> ParsedBoundaryCreateBody | V1HttpResponse:
    ...
    if parsed["mode"] != "fail_closed":
        return _error(400, "invalid_request", "boundary mode must be fail_closed")
    return ParsedBoundaryCreateBody(
        name=parsed["name"],
        runtime=parsed["runtime"],
        boundary_type=parsed["boundary_type"],
        mode="fail_closed",
