...
class LocalLaunchConfig:
    ...
    boundary_name: str | None = "claude-code-local"
    boundary_runtime: str = "claude-code"
    boundary_type: str = "pretooluse"
    idempotency_keyring: IdempotencyKeyring | None = field(
        default_factory=lambda: load_idempotency_keyring(os.environ),
        repr=False,
    )
...
def _parse_args(argv: list[str] | None) -> LocalLaunchConfig:
    ...
    parser.add_argument(
        ...
        help="Local boundary name to create or reuse.",
    )
