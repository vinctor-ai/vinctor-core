...
from vinctor_service.audit import (
...
class InMemoryV1Service:
    ...
    def audit_event_sequences(
        ...
        return {
            ...
            for sequence, event in enumerate(self.audit_writer.events, start=1)
            if event.workspace_id == workspace_id and event.event_id in requested
        }
        ...
        *,
        ...
        if snapshot_event_id is not None:
            snapshot_position = next(
                (
                    position
                    for position, event in enumerate(self.audit_writer.events, start=1)
        ...
        if before_event_id is not None:
            ...
            if before_position is None:
                raise InvalidAuditCursorError
        if (
            snapshot_position is not None
            and before_position is not None
            and before_position > snapshot_position
        ):
            raise InvalidAuditCursorError
        snapshot_bound = (
            snapshot_position
            if snapshot_position is not None
            else len(self.audit_writer.events)
        )
        before_bound = (
            before_position
            if before_position is not None
            else len(self.audit_writer.events) + 1
        )
        matched = [
            event
            for position, event in enumerate(self.audit_writer.events, start=1)
            if position <= snapshot_bound
            and position < before_bound
            and event.workspace_id == workspace_id
