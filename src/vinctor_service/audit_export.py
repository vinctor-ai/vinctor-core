...
from vinctor_service.audit import AuditWriter, BestEffortAuditWriter
from vinctor_service.audit_sink import AuditSinkConfigurationError, _probe_file_destination
...
_STOP = object()
_INVALID_OTLP_ENDPOINT = "OTLP HTTP endpoint must be a valid http(s) URL"
...
def _otlp_record(event: AuditEvent) -> dict[str, object]:
    ...
    return {
        "timeUnixNano": str(
            int(event.created_at.timestamp()) * 1_000_000_000 + event.created_at.microsecond * 1_000
        ),
...
def _otlp_json(events: list[AuditEvent]) -> bytes:
    payload = {
        "resourceLogs": [
            {
                "resource": {
                    "attributes": [{"key": "service.name", "value": {"stringValue": "vinctor"}}]
                },
                "scopeLogs": [
                    {
...
class OtlpHttpExport:
    ...
    def __init__(
        ...
        *,
        ...
        if any(
            ...
            raise ValueError(_INVALID_OTLP_ENDPOINT)
        try:
            ...
            raise ValueError(_INVALID_OTLP_ENDPOINT) from None
        authority = parsed.netloc.rsplit("@", 1)[-1]
        if authority.startswith("["):
            port_suffix = authority[authority.rfind("]") + 1 :]
            authority_shape_valid = not port_suffix or port_suffix.startswith(":")
            has_explicit_port = port_suffix.startswith(":")
        else:
        ...
        if (
            ...
            or ("@" in parsed.netloc and not username)
            or not authority_shape_valid
            or (has_explicit_port and port is None)
    ...
    def emit(self, event: AuditEvent) -> None:
        with self._state_lock:
            if self._closed:
                sys.stderr.write("vinctor: audit export is closed (otlp-http); event dropped\n")
                return
            try:
        ...
        for attempt in range(1, self._max_attempts + 1):
            try:
                ...
                if attempt >= self._max_attempts or not _is_transient(exc):
                    sys.stderr.write(f"vinctor: audit export emit failed (otlp-http): {exc}\n")
                    return
                try:
                    self._sleeper(self._retry_backoff * (2 ** (attempt - 1)))
                except Exception as sleep_error:
                    sys.stderr.write(
                        f"vinctor: audit export retry wait failed (otlp-http): {sleep_error}\n"
                    )
                    return
...
class ExportingAuditWriter:
    ...
    def write(self, event: AuditEvent) -> None:
        ...
        self._schedule_export(event)

    def write_best_effort(self, event: AuditEvent) -> bool:
        try:
            if isinstance(self._wrapped, BestEffortAuditWriter):
                persisted = self._wrapped.write_best_effort(event)
            else:
                self._wrapped.write(event)
                persisted = True
        except Exception:
    ...
    def _schedule_export(self, event: AuditEvent) -> None:
...
def audit_export_from_env(env: dict[str, str]) -> AuditExport:
    ...
    if spec.startswith("file:"):
        path = spec[len("file:") :]
        _probe_file_destination(path, "VINCTOR_AUDIT_EXPORT")
        return FileExport(path)
    if spec.startswith("otlp-http:"):
        ...
        try:
            return OtlpHttpExport(
                ...
                max_attempts=_env_int(env, "VINCTOR_AUDIT_EXPORT_MAX_ATTEMPTS", 3),
                retry_backoff=_env_float(env, "VINCTOR_AUDIT_EXPORT_RETRY_BACKOFF_SECONDS", 0.1),
            )
        except ValueError:
    ...
    raise AuditSinkConfigurationError(
        "VINCTOR_AUDIT_EXPORT",
        "unsupported sink configuration",
    )
