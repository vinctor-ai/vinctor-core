...
def _create_rule(
    ...
    *,
    ...
    parsed = parse_v1_auto_approval_rule_create_body(body)
    if isinstance(parsed, V1HttpResponse):
        return parsed

    return create_v1_auto_approval_rule_response(
        parsed,
        identity=identity,
        service=service,
        now=now,
...
def create_v1_auto_approval_rule_response(
    parsed: ParsedCreateRuleBody,
    *,
    identity: WorkspaceIdentity,
    service: AutoApprovalAdminService,
    now: datetime,
...
def parse_v1_auto_approval_rule_create_body(
    body: object,
) -> ParsedCreateRuleBody | V1HttpResponse:
