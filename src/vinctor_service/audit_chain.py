...
class AnchorVerification:
    """Result of checking a chain against an external anchor log.

    ``covered_max_seq`` is the highest chain row the anchor actually asserts.
    Rows ABOVE it are unanchored: verify_chain proves only that they are
    internally consistent, which an attacker who suppressed the (fail-open)
    anchor sink can arrange for a rewritten tail. Coverage that stops below
    ``head_seq`` is therefore a verification FAILURE — divergence_kind
    ``uncovered``, with ``divergence_seq`` the first unanchored row.
    """
    ...
    divergence_kind: str | None = None  # missing | mismatch | uncovered
