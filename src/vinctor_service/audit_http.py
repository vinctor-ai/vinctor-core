...
from vinctor_service.audit import InvalidAuditCursorError
    ...
    *,
    ...
    if path == "/v1/audit-events":
        ...
        if isinstance(filters, V1HttpResponse):
            return filters
        cursor_filters: dict[str, str | None] = {}
        if filters.paginate:
            cursor_filters = {
                "snapshot_event_id": filters.snapshot_event_id,
                "before_event_id": filters.before_event_id,
            }
        try:
            events = service.list_filtered(
                ...
                limit=filters.limit + 1 if filters.paginate else filters.limit,
                **cursor_filters,
            )
        except InvalidAuditCursorError:
            return _error(400, "invalid_request", "invalid cursor")
        if filters.paginate:
            has_more = len(events) > filters.limit
            page_events = events[-filters.limit :] if has_more else events
            sequences = service.audit_event_sequences(
                identity.workspace_id,
                tuple(event.event_id for event in page_events),
            )
            snapshot_event_id = filters.snapshot_event_id
            if snapshot_event_id is None and page_events:
                snapshot_event_id = page_events[-1].event_id
            return V1HttpResponse(
                status_code=200,
                body={
                    "audit_events": [
                        {
                            **_audit_event_body(event),
                            "sequence": sequences[event.event_id],
                        }
                    ...
                    "page": {
                        ...
                        "next_before_event_id": (
                            page_events[0].event_id if has_more else None
                        ),
                    },
                },
            )
...
def _parse_filters(query_string: str) -> AuditEventFilters | V1HttpResponse:
    ...
    paginate = values["page"] == "true"
    if values["page"] is not None and not paginate:
        return _error(400, "invalid_request", "page must be true")
    if not paginate and (
        values["snapshot_event_id"] is not None
        or values["before_event_id"] is not None
    ):
        return _error(
            400,
            "invalid_request",
            "snapshot_event_id and before_event_id require page=true",
        )
    if (
        values["before_event_id"] is not None
        and values["snapshot_event_id"] is None
    ):
        return _error(
            400,
            "invalid_request",
    ...
    return AuditEventFilters(
        ...
        paginate=paginate,
        snapshot_event_id=values["snapshot_event_id"],
        before_event_id=values["before_event_id"],
        limit=limit,
    )
