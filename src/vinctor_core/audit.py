...
# Subject-token rejection causes. The AGENT-facing response is one identical
# generic 403 for all of them (see v1_enforce) — a caller must never learn which
# check it tripped, or it can map the token/PEP configuration by probing. The
# operator audit is the opposite surface, and these remedies are all different:
# re-mint (not found / expired), stop (revoked), fix the PEP or the mint
# parameters (mismatch), fix the clock or the shared secret (proof invalid).
REASON_SUBJECT_TOKEN_NOT_FOUND = "subject_token_not_found"
REASON_SUBJECT_TOKEN_REVOKED = "subject_token_revoked"
REASON_SUBJECT_TOKEN_EXPIRED = "subject_token_expired"
REASON_SUBJECT_TOKEN_MISMATCH = "subject_token_mismatch"
REASON_POP_PROOF_INVALID = "pop_proof_invalid"
