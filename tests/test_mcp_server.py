...
def test_server_module_entrypoint_invokes_main() -> None:
    ...
    # `main` ran: a clean one-line error on stderr, not a traceback.
    assert result.stderr.startswith("error: ")
    assert result.stderr.strip().count("\n") == 0, f"expected one line: {result.stderr!r}"
    assert "Traceback (most recent call last)" not in result.stderr

    # WHICH error depends on whether the [mcp] extra is installed here, so this
    # assertion must not hardcode one. The SDK check deliberately precedes
    # config validation: a bare install names the extra and never the env var.
    # Asserting the env var unconditionally passed only because a dev venv has
    # the SDK, and hid that ordering entirely from every environment but CI's.
    if importlib.util.find_spec("mcp") is None:
        assert "MCP SDK is required to run vinctor-mcp-server." in result.stderr
        assert "vinctor-core[mcp]" in result.stderr
        assert "VINCTOR_MCP_ENDPOINT" not in result.stderr
    else:
        assert "VINCTOR_MCP_ENDPOINT is required" in result.stderr
