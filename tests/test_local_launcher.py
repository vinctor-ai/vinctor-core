...
def test_default_local_service_provisions_required_boundary(tmp_path: Path) -> None:
    handle = prepare_local_service(
        LocalLaunchConfig(db_path=tmp_path / "vinctor.sqlite", port=0),
        now=NOW,
    )
    try:
        assert handle.boundary is not None
        assert handle.boundary.name == "claude-code-local"
        assert 'export VINCTOR_BOUNDARY_ID="' in render_env_exports(handle, now=NOW)
    finally:
        handle.close()
