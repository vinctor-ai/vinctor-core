...
def test_anchor_from_env_selects_sink(tmp_path: Path) -> None:
    file_path = tmp_path / "anchor.jsonl"
    assert isinstance(anchor_from_env({}), NullAnchor)
    assert isinstance(anchor_from_env({"VINCTOR_AUDIT_ANCHOR": ""}), NullAnchor)
    assert isinstance(
        anchor_from_env({"VINCTOR_AUDIT_ANCHOR": f"file:{file_path}"}), FileAnchor
    )
