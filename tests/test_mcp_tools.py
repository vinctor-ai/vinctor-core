...
class ReportClient(FakeClient):
    ...
    def list_audit_events(
        ...
        *,
        ...
        for sequence, audit_event in enumerate(events, start=1):
            audit_event["sequence"] = sequence
        if boundary_id is not None:
            events = [
                audit_event
                for audit_event in events
                if audit_event["event_type"]
                not in {"grant_issued", "grant_revoked"}
            ]
        body: dict[str, Any] = {"audit_events": events}
        if paginate:
            body["page"] = {
                "snapshot_event_id": snapshot_event_id or events[-1]["event_id"],
                "next_before_event_id": None,
            }
        return body


class CappedReportClient(ReportClient):
        ...
        self._events = [
            {
                ...
                "workspace_id": "ws_main",
                "agent_id": "agent_release",
                "grant_id": "grnt_complete",
                "grant_ref": "grt_complete",
                "action": "send",
                "resource": "email/external",
                "boundary_id": "bnd_complete",
                "created_at": f"2026-06-11T12:00:{index % 60:02d}+00:00",
                "sequence": index + 1,
            }
    ...
    def list_audit_events(
        ...
        *,
        ...
        matching = [
            ...
            if (grant_ref is None or event["grant_ref"] == grant_ref)
            and (boundary_id is None or event["boundary_id"] == boundary_id)
        ]
        snapshot_index = (
            next(
                index
        ...
        before_index = (
            ...
            else snapshot_index + 1
        )
        bounded = matching[: min(snapshot_index + 1, before_index)]
        events = bounded[-limit:]
        body: dict[str, Any] = {"audit_events": events}
        if paginate:
            body["page"] = {
                "snapshot_event_id": (
                    matching[snapshot_index]["event_id"] if matching else None
                ),
                "next_before_event_id": (
                    events[0]["event_id"] if len(bounded) > limit else None
                ),
            }
        return body


def test_grant_report_includes_more_than_the_default_audit_page() -> None:
    tools = VinctorReadOnlyTools(CappedReportClient(event_count=25))

    report = tools.grant_report("grt_complete")

    assert [event["event_id"] for event in report["usage"]] == [
        f"evt_{index:03d}" for index in range(25)
    ]


def test_grant_report_includes_more_than_the_audit_hard_cap() -> None:
    client = CappedReportClient(event_count=125)

    report = VinctorReadOnlyTools(client).grant_report("grt_complete")

    assert [event["event_id"] for event in report["usage"]] == [
        f"evt_{index:03d}" for index in range(125)
    ]
    assert [call["limit"] for call in client.audit_calls] == [100, 100]


def test_boundary_report_counts_more_than_the_default_audit_page() -> None:
    report = VinctorReadOnlyTools(
        CappedReportClient(event_count=25)
    ).boundary_report("bnd_complete")

    assert report["activity"] == {"permit": 25, "deny": 0}
    assert len(report["recent"]) == 25


def test_grant_request_report_includes_more_than_the_audit_hard_cap() -> None:
    report = VinctorReadOnlyTools(
        CappedReportClient(event_count=125)
    ).grant_request_report("grq_complete")
...
def test_report_paging_excludes_a_concurrently_appended_tail() -> None:
    class GrowingReportClient(CappedReportClient):
        def list_audit_events(self, **filters: Any) -> dict[str, Any]:
            body = super().list_audit_events(**filters)
            if (
                filters.get("paginate") is True
                and filters.get("snapshot_event_id") is None
            ):
                self._events.append(
                    {
                        **self._events[-1],
    ...
    client = GrowingReportClient(event_count=125)

    report = VinctorReadOnlyTools(client).grant_report("grt_complete")

    assert client.audit_calls[1]["snapshot_event_id"] == "evt_124"
    assert client.audit_calls[1]["before_event_id"] == "evt_025"
    assert [event["event_id"] for event in report["usage"]] == [
        f"evt_{index:03d}" for index in range(125)
    ]
