...
def test_sqlite_set_bounds_audit_reason_distinguishes_preserve_and_clear() -> None:
    service = _sqlite_service()
    service.set_agent_issuable_scope_bounds(
        workspace_id="ws_main", agent_id="agent_a",
        scopes=("read:repo/project/*",),
        max_ttl_seconds=3600,
        now=NOW,
    )
    # No max TTL declared: the stored cap is preserved, and the audit trail
    # must say so rather than claim the cap is gone.
    service.set_agent_issuable_scope_bounds(
        workspace_id="ws_main", agent_id="agent_a",
        scopes=("read:repo/project/*",),
        now=NOW,
    ...
    assert [e.reason for e in events] == [
        "max_ttl_seconds=3600",
        "max_ttl_seconds=preserved",
        "max_ttl_seconds=cleared",
    ]
