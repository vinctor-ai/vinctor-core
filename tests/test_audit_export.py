...
def test_export_from_env_selects_sink(tmp_path: Path) -> None:
    ...
    assert isinstance(
        audit_export_from_env({"VINCTOR_AUDIT_EXPORT": f"file:{file_path}"}), FileExport
    )
...
def test_export_from_env_rejects_invalid_otlp_delivery_config() -> None:
    with pytest.raises(ValueError, match="VINCTOR_AUDIT_EXPORT") as exc_info:
        audit_export_from_env(
            {
                "VINCTOR_AUDIT_EXPORT": "otlp-http:http://collector:4318/v1/logs",
    ...
    assert "zero" not in str(exc_info.value)


@pytest.mark.parametrize(
    "value",
    ["nan", "NaN", "-nan", "inf", "Infinity", "-inf", "-Infinity", "1e9999"],
)
def test_export_from_env_rejects_non_finite_retry_backoff(value: str) -> None:
    with pytest.raises(ValueError, match="VINCTOR_AUDIT_EXPORT") as exc_info:
        audit_export_from_env(
            {
                "VINCTOR_AUDIT_EXPORT": "otlp-http:http://collector:4318/v1/logs",
                "VINCTOR_AUDIT_EXPORT_RETRY_BACKOFF_SECONDS": value,
            }
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "http://:4318/v1/logs",
        "http://user:pass@/v1/logs",
        "http://@/v1/logs",
        "http://user@:4318/v1/logs",
        "http://:pass@collector/v1/logs",
        "http://collector:bad/v1/logs",
        "http://collector:65536/v1/logs",
        "http://collector:-1/v1/logs",
        "http://collector:/v1/logs",
        "http://collector:0/v1/logs",
        "http://collector:%34%33%31%38/v1/logs",
        "http://collector\n.example/v1/logs",
        "http://collector\t.example/v1/logs",
        "http://collector\x00.example/v1/logs",
        "http://collector .example/v1/logs",
        "http://%63ollector:4318/v1/logs",
        "http://[::1/v1/logs",
    ],
)
def test_export_from_env_rejects_invalid_otlp_http_endpoint(endpoint: str) -> None:
    with pytest.raises(ValueError, match="VINCTOR_AUDIT_EXPORT") as exc_info:
        audit_export_from_env(
            {"VINCTOR_AUDIT_EXPORT": f"otlp-http:{endpoint}"}
        )
...
@pytest.mark.parametrize(
    ...
    [
        "http://collector:4318/v1/logs",
        "https://127.0.0.1:4318/v1/logs?tenant=demo",
        "http://[::1]:4318/v1/logs",
        "https://collector/v1/logs",
        "https://user:pass@collector:4318/v1/logs",
    ],
)
def test_export_from_env_accepts_valid_otlp_http_endpoint(endpoint: str) -> None:
    export = audit_export_from_env(
        {"VINCTOR_AUDIT_EXPORT": f"otlp-http:{endpoint}"}
    )
    assert isinstance(export, OtlpHttpExport)
...
def test_export_from_env_rejects_unknown_spec() -> None:
    with pytest.raises(ValueError, match="VINCTOR_AUDIT_EXPORT"):
        audit_export_from_env({"VINCTOR_AUDIT_EXPORT": "otlp:collector:4317"})
...
def test_sqlite_service_writer_is_unwrapped_when_env_unset(monkeypatch, capsys) -> None:
