...
from vinctor_service.models import AgentIssuableBounds, GrantIssueRequest
...
""",
    ...
    assert get_sqlite_schema_versions(service.conn) == tuple(range(1, 19))
...
def test_policy_apply_without_max_ttl_preserves_existing_cap(tmp_path: Path) -> None:
    service = SQLiteV1Service(connect_sqlite(":memory:"))
...
def test_policy_apply_declared_max_ttl_sets_and_explicit_null_clears_cap(
    tmp_path: Path,
) -> None:
    service = SQLiteV1Service(connect_sqlite(":memory:"))
...
def test_policy_apply_rejects_bounds_entry_with_both_max_ttl_keys(
    tmp_path: Path,
) -> None:
    service = SQLiteV1Service(connect_sqlite(":memory:"))

    with pytest.raises(ValueError, match="must not set both max_ttl and max_ttl_seconds"):
        apply_policy_file(
            _write_policy(
                tmp_path / "policy.yaml",
                """
...
def test_policy_export_round_trips_agent_bounds_max_ttl(tmp_path: Path) -> None:
    service = SQLiteV1Service(connect_sqlite(":memory:"))
    service.set_agent_issuable_scope_bounds(
        workspace_id="ws_main",
        agent_id="agent_a",
