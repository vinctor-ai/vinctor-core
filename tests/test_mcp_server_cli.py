...
def bare_install(monkeypatch: pytest.MonkeyPatch) -> None:
    """A install with no ``[mcp]`` extra and no ``VINCTOR_*`` configuration.
    ...
    EVERY ``VINCTOR_*`` name is cleared, not just ``VINCTOR_MCP_*``. Since
    PKA-203 ``main`` also emits the unrecognized-variable warning, so one stray
    name in the developer's shell would add a second stderr line and break the
    one-line assertion below — a real assertion failing for a reason that has
    ...
    for name in [n for n in os.environ if n.startswith("VINCTOR_")]:
