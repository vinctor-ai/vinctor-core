...
def test_postgres_set_bounds_partial_update_preserves_and_clears_max_ttl() -> None:
    assert DSN is not None
    conn = connect_postgres(DSN)
    repository = _bounds_repo(conn)
    repository.set_bounds(
    ...
    # Re-declaring the bounds WITHOUT a max TTL must not silently widen the
    # agent's issuable authority by clearing the operator's cap (PKA-102).
    repository.set_bounds(
        workspace_id="ws_main",
        agent_id="agent_release",
        scopes=("write:repo/feature/*",),
    ...
    assert repository.get_bounds_with_max_ttl(
        workspace_id="ws_main", agent_id="agent_release"
    ) == AgentIssuableBounds(scopes=("write:repo/feature/*",), max_ttl_seconds=600)

    # Removing the cap is an explicit act, never an omission.
    repository.set_bounds(
        workspace_id="ws_main",
        agent_id="agent_release",
        scopes=("write:repo/feature/*",),
    ...
    assert repository.get_bounds_with_max_ttl(
        workspace_id="ws_main", agent_id="agent_release"
    ) == AgentIssuableBounds(scopes=("write:repo/feature/*",), max_ttl_seconds=None)

    with pytest.raises(ValueError, match="clear_max_ttl"):
        repository.set_bounds(
            workspace_id="ws_main",
            agent_id="agent_release",
            scopes=("write:repo/feature/*",),
...
def test_postgres_workspace_override_is_visible_to_get_require_boundary() -> None:
    # Parity with SQLite: get_require_boundary reports the EFFECTIVE mandate and
    # must agree with is_boundary_required. Reading only the agent row and
    # falling back to the schema default skipped the workspace override.
    assert DSN is not None
    conn = connect_postgres(DSN)
    repo = _settings_repo(conn)
    repo.set_require_boundary(
        workspace_id="ws_main", agent_id="", require_boundary=False, now=NOW
    )
    # Fresh schema default is TRUE, so a workspace-wide OFF is exactly the case
    # where reading the schema default would wrongly claim the mandate is on.
    assert repo.is_boundary_required(workspace_id="ws_main", agent_id="agent_other") is False
    assert repo.get_require_boundary(workspace_id="ws_main", agent_id="agent_other") is False

    repo.set_require_boundary(
        workspace_id="ws_main", agent_id="", require_boundary=True, now=NOW
    )
    assert repo.is_boundary_required(workspace_id="ws_main", agent_id="agent_other") is True
    assert repo.get_require_boundary(workspace_id="ws_main", agent_id="agent_other") is True
    conn.close()
...
def test_postgres_fresh_schema_defaults_boundary_required_and_allows_explicit_off() -> None:
    assert DSN is not None
    conn = connect_postgres(DSN)
    ...
    assert default_row is not None and str(default_row[0]).lower() == "true"
    assert repo.get_require_boundary(
        workspace_id="ws_fresh", agent_id="agent_fresh"
    ) is True
...
def test_postgres_enforcement_presence_migration_fail_closed() -> None:
    ...
    assert default_row is not None and str(default_row[0]).lower() == "false"
    assert repo.get_require_boundary(
        workspace_id="ws_legacy", agent_id="agent_legacy"
    ) is False
    assert repo.is_boundary_required(
        workspace_id="ws_legacy", agent_id="agent_legacy"
    ) is False
...
def _thirty_day_issue_request() -> GrantIssueRequest:
    return GrantIssueRequest(
        workspace_id="ws_main",
        target_agent_id="agent_a",
        requested_scopes=("read:repo/a",),
        ttl_seconds=30 * 24 * 3600,
    )


def test_postgres_policy_apply_without_max_ttl_preserves_existing_cap(
    tmp_path,
) -> None:
    assert DSN is not None
    conn = connect_postgres(DSN)
...
""".strip(),
    ...
    # PKA-102: a policy silent on the cap must not widen authority by
    # resetting the operator's issuance cap to NULL.
    apply_policy_file(
        policy_path,
        service=service,
        workspace_id="ws_main",
    ...
    assert service.scope_bounds_repository.get_bounds_with_max_ttl(
        workspace_id="ws_main", agent_id="agent_a"
    ) == AgentIssuableBounds(scopes=("read:repo/a",), max_ttl_seconds=600)
    rejected = service.issue_grant(_thirty_day_issue_request(), now=NOW)
    assert (rejected.status, rejected.reason) == ("rejected", "ttl_exceeds_issuable_max")
    conn.close()


def test_postgres_policy_apply_explicit_null_max_ttl_clears_cap(tmp_path) -> None:
    assert DSN is not None
    conn = connect_postgres(DSN)
    service = PostgresV1Service(conn)
    service.set_agent_issuable_scope_bounds(
...
""".strip(),
    ...
    assert service.scope_bounds_repository.get_bounds_with_max_ttl(
        workspace_id="ws_main", agent_id="agent_a"
    ) == AgentIssuableBounds(scopes=("read:repo/a",), max_ttl_seconds=None)
    assert service.issue_grant(_thirty_day_issue_request(), now=NOW).status == "issued"
    conn.close()


def test_postgres_policy_export_round_trips_agent_bounds_max_ttl(tmp_path) -> None:
    assert DSN is not None
    conn = connect_postgres(DSN)
    service = PostgresV1Service(conn)
    service.set_agent_issuable_scope_bounds(
    ...
    document = export_policy_document(service=service, workspace_id="ws_main")

    # The cap is part of the exported grammar; an uncapped agent omits the key
    # (absent = preserve on re-apply, so an export never silently clears).
    assert document["agent_bounds"] == [
        {"agent_id": "agent_a", "scopes": ["read:repo/a"], "max_ttl": "600s"},
        {"agent_id": "agent_b", "scopes": ["read:repo/b"]},
    ]

    exported = tmp_path / "exported.yaml"
    write_policy_file(exported, document)
    apply_policy_file(
        exported,
        service=service,
        workspace_id="ws_main",
    ...
    ) == AgentIssuableBounds(scopes=("read:repo/b",), max_ttl_seconds=None)
    rejected = service.issue_grant(_thirty_day_issue_request(), now=NOW)
    assert (rejected.status, rejected.reason) == ("rejected", "ttl_exceeds_issuable_max")
    conn.close()
...
def test_postgres_audit_list_applies_snapshot_and_before_cursors() -> None:
    assert DSN is not None
    conn = connect_postgres(DSN)
    writer = _seed_pg_chain(conn, count=5)

    newest = writer.list_filtered("ws_main", grant_ref="grt_1", limit=3)
    writer.write(_pg_audit_event("evt_concurrent"))
    older = writer.list_filtered(
        "ws_main",
        grant_ref="grt_1",
        snapshot_event_id="evt_5",
        before_event_id="evt_3",
        limit=3,
    )

    assert [event.event_id for event in newest] == ["evt_3", "evt_4", "evt_5"]
    assert [event.event_id for event in older] == ["evt_1", "evt_2"]
    conn.close()


def test_postgres_audit_event_sequences_are_global_and_workspace_scoped() -> None:
    assert DSN is not None
    conn = connect_postgres(DSN)
    writer = PostgresAuditWriter(conn)
    writer.write(_pg_audit_event("evt_first"))
    writer.write(
        replace(_pg_audit_event("evt_other"), workspace_id="ws_other")
    )
    writer.write(_pg_audit_event("evt_third"))

    sequences = writer.event_sequences(
        "ws_main",
...
def test_postgres_audit_list_rejects_invalid_cursor_without_disclosure(
    snapshot_event_id: str | None,
    before_event_id: str | None,
) -> None:
    # Given: a PostgreSQL chain with valid same-workspace and cross-workspace cursors.
    assert DSN is not None
    conn = connect_postgres(DSN)
    writer = _seed_pg_chain(conn)
    writer.write(
        replace(
            _pg_audit_event("evt_other"),
            workspace_id="ws_other",
        )
    )

    # When/Then: every invalid cursor class raises the same coarse store error.
    with pytest.raises(ValueError, match="^invalid cursor$"):
        writer.list_filtered(
            "ws_main",
...
def test_postgres_audit_writer_default_is_unwrapped_and_silent(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    assert DSN is not None
    _clear_postgres_audit_sink_env(monkeypatch)
    service = PostgresV1Service(connect_postgres(DSN))
    try:
        assert type(service.audit_writer) is PostgresAuditWriter
        assert isinstance(service.audit_writer._anchor, NullAnchor)
...
def _clear_postgres_audit_sink_env(monkeypatch: pytest.MonkeyPatch) -> None:
...
class _RaisingAuditExport:
    def emit(self, event: AuditEvent) -> None:
        raise RuntimeError("sink down")


def _postgres_service_with_raising_export(
    monkeypatch: pytest.MonkeyPatch,
) -> PostgresV1Service:
    assert DSN is not None
    _clear_postgres_audit_sink_env(monkeypatch)
    ...
    service = PostgresV1Service(connect_postgres(DSN))
    assert isinstance(service.audit_writer, ExportingAuditWriter)
    service.audit_writer._export = _RaisingAuditExport()
    return service


def test_postgres_startup_rejects_unknown_sink_even_when_other_sink_is_valid(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # Given
    assert DSN is not None
    ...
    try:
        with pytest.raises(ValueError, match="VINCTOR_AUDIT_EXPORT"):
            PostgresV1Service(conn)
    finally:
        conn.close()


def test_postgres_startup_rejects_empty_file_sink_path(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # Given
    assert DSN is not None
...
def test_postgres_startup_rejects_unopenable_file_sink_destination(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # Given
...
def test_postgres_required_audit_sink_profile_rejects_missing_sinks(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # Given
    assert DSN is not None
...
def test_postgres_required_audit_sink_profile_accepts_valid_export(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # Given
    assert DSN is not None
...
def test_postgres_runtime_raising_export_does_not_block_decision_or_chain(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    # Given
    service = _postgres_service_with_raising_export(monkeypatch)
    service.insert_grant(grant())
...
def test_postgres_runtime_raising_export_does_not_roll_back_state(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    # Given
    ...
    request = GrantRequestCreateRequest(
        ...
        reason="run CI",
        request_id="grq_sink_down",
    )

    # When
    try:
        created = service.create_grant_request(request, now=NOW)
...
def test_postgres_chain_verifies_after_sink_misconfiguration_is_corrected(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # Given
    ...
    _clear_postgres_audit_sink_env(monkeypatch)
    monkeypatch.setenv("VINCTOR_AUDIT_EXPORT", "unknown:sink")
    rejected_conn = connect_postgres(DSN)
    try:
        with pytest.raises(ValueError, match="VINCTOR_AUDIT_EXPORT"):
            PostgresV1Service(rejected_conn)
    finally:
        rejected_conn.close()
