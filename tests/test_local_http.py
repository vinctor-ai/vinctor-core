...
@pytest.mark.parametrize(
    ("unavailable", "backend"),
    [
        (SQLitePoolUnavailable(), "sqlite"),
        (PostgresConnectionUnavailable("PostgreSQL connection is unavailable"), "postgres"),
    ],
)
def test_backend_unavailable_sheds_load_as_503_on_both_backends(
    unavailable: Exception, backend: str
) -> None:
    # Both backends must fail CLOSED the same way. `_dispatch` used to catch
    # SQLitePoolUnavailable only, so on Postgres a connection failure escaped
    # the handler: the client got a dropped connection with no response at all
    # instead of a 503 (PKA-232).
    @contextmanager
    def scope() -> Iterator[None]:
        raise unavailable
    ...
    with running_server(service(), request_scope=scope) as server:
        status, body = post_json(server)

    assert status == 503, backend
