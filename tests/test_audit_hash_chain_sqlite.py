...
def test_backfill_chains_existing_rows_from_genesis(tmp_path) -> None:
    ...
    index_columns = conn.execute(
        "PRAGMA index_xinfo(idx_audit_events_workspace_seq)"
    ).fetchall()
    assert [(row[2], row[3]) for row in index_columns if row[5]] == [
        ("workspace_id", 0),
        ("seq", 1),
    ]
...
def test_chain_stays_valid_under_concurrent_enforce(tmp_path) -> None:
    ...
    # 403 = the boundary + bounds bootstrap control events, the grant-issued
    # decision event, and the 400 concurrent enforce events.
    assert v.ok is True and v.count == 403 and v.head_seq == 403
...
def test_v14_rows_with_legacy_identity_key_migrate_and_still_verify(tmp_path) -> None:
    ...
    init_sqlite_schema(conn)  # v14 -> current schema (v18)
