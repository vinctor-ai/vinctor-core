...
def test_sqlite_fresh_schema_defaults_boundary_required_and_allows_explicit_off(
    tmp_path,
) -> None:
    conn = connect_sqlite(tmp_path / "v.sqlite")
    init_sqlite_schema(conn)
    repo = _sqlite_repo(conn)
    schema_default = conn.execute(
        """
        SELECT dflt_value
    ...
    assert schema_default is not None and schema_default[0] == "1"
    assert repo.get_require_boundary(workspace_id="ws", agent_id="a") is True
    assert repo.is_boundary_required(workspace_id="ws", agent_id="a") is True

    repo.set_require_boundary(
        workspace_id="ws", agent_id="a", require_boundary=False, now=NOW
    )
    assert repo.get_require_boundary(workspace_id="ws", agent_id="a") is False
    assert repo.is_boundary_required(workspace_id="ws", agent_id="a") is False


def test_sqlite_migrated_schema_keeps_default_boundary_off(tmp_path) -> None:
    conn = connect_sqlite(tmp_path / "v.sqlite")
    conn.executescript(
        """
    ...
    assert schema_default is not None and schema_default[0] == "0"
    assert repo.get_require_boundary(workspace_id="ws", agent_id="a") is False
    assert repo.is_boundary_required(workspace_id="ws", agent_id="a") is False
...
def _migrated_schema_conn(tmp_path, name: str):
    # A database first created before 0.6.0: CREATE TABLE IF NOT EXISTS never
    # rewrites it, so the require_boundary column default stays 0 forever.
...
def test_workspace_override_is_visible_to_get_require_boundary(tmp_path) -> None:
    # `operator require-boundary show <agent>` calls get_require_boundary, and an
    # operator reads its output as "will a header-less call be denied?". It must
    # therefore resolve all three layers, exactly as is_boundary_required does.
    # Reading only the agent row and falling back to the schema default skipped
    # the workspace override in between, so `show` contradicted enforce.

    # Migrated database (schema default 0), workspace mandate turned ON: the
    # rollout worked, so `show` must not report the agent as exempt.
    conn = _migrated_schema_conn(tmp_path, "migrated.sqlite")
    repo = _sqlite_repo(conn)
    repo.set_require_boundary(workspace_id="ws", agent_id="", require_boundary=True, now=NOW)
    assert repo.is_boundary_required(workspace_id="ws", agent_id="a") is True
    assert repo.get_require_boundary(workspace_id="ws", agent_id="a") is True

    # Fresh 0.6.0 database (schema default 1), workspace mandate turned OFF: the
    # mandate really is off, so `show` must not claim it is on. This is the
    # dangerous direction — false assurance about a security control.
    fresh = connect_sqlite(tmp_path / "fresh.sqlite")
    init_sqlite_schema(fresh)
    fresh_repo = _sqlite_repo(fresh)
    fresh_repo.set_require_boundary(
        workspace_id="ws", agent_id="", require_boundary=False, now=NOW
    )
    assert fresh_repo.is_boundary_required(workspace_id="ws", agent_id="a") is False
    assert fresh_repo.get_require_boundary(workspace_id="ws", agent_id="a") is False

    # An agent override still outranks the workspace override in both readers.
    fresh_repo.set_require_boundary(
        workspace_id="ws", agent_id="a", require_boundary=True, now=NOW
    )
    assert fresh_repo.is_boundary_required(workspace_id="ws", agent_id="a") is True
    assert fresh_repo.get_require_boundary(workspace_id="ws", agent_id="a") is True


def test_in_memory_workspace_override_is_visible_to_get_require_boundary() -> None:
    repo = InMemoryAgentEnforcementSettingsRepository()
    repo.set_require_boundary(workspace_id="ws", agent_id="", require_boundary=True, now=NOW)
    assert repo.is_boundary_required(workspace_id="ws", agent_id="a") is True
    assert repo.get_require_boundary(workspace_id="ws", agent_id="a") is True
