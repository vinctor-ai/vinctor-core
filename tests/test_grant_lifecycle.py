...
def test_whitespace_agent_id_preserves_direct_issue_contract(tmp_path: Path) -> None:
    conn = connect_db(tmp_path)
    service = SQLiteV1Service(conn)
    service.set_agent_issuable_scope_bounds(
        workspace_id="ws_main",
    ...
    assert result.grant.agent_id == "agent runner"
    conn.close()
...
def test_set_bounds_without_max_ttl_preserves_existing_cap(tmp_path: Path) -> None:
    conn = connect_db(tmp_path)
    service = SQLiteV1Service(conn)
    service.set_agent_issuable_scope_bounds(
        workspace_id="ws_main",
    ...
    assert service.scope_bounds_repository.get_bounds_with_max_ttl(
        workspace_id="ws_main", agent_id="agent_runner"
    ) == AgentIssuableBounds(scopes=("execute:ci/test",), max_ttl_seconds=600)
    rejected = service.issue_grant(issue_request(ttl_seconds=3600), now=NOW)
    assert (rejected.status, rejected.reason) == ("rejected", "ttl_exceeds_issuable_max")
    conn.close()


def test_set_bounds_clear_max_ttl_explicitly_removes_cap(tmp_path: Path) -> None:
    conn = connect_db(tmp_path)
    service = SQLiteV1Service(conn)
    service.set_agent_issuable_scope_bounds(
        workspace_id="ws_main",
    ...
    assert service.scope_bounds_repository.get_bounds_with_max_ttl(
        workspace_id="ws_main", agent_id="agent_runner"
    ) == AgentIssuableBounds(scopes=("execute:ci/test",), max_ttl_seconds=None)
    assert service.issue_grant(issue_request(ttl_seconds=3600), now=NOW).status == "issued"
    conn.close()


def test_in_memory_set_bounds_matches_durable_partial_update_semantics() -> None:
    repository = InMemoryAgentIssuableScopeBoundsRepository()
    ...
    assert (
        repository.get_max_ttl_seconds(workspace_id="ws_main", agent_id="agent_runner")
        == 600
    )
