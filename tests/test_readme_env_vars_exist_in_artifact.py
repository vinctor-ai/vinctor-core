    ...
    for root in _wheel_source_roots():
        for path in sorted(root.rglob("*")):
            if not path.is_file() or "__pycache__" in path.parts:
                continue
            # PKA-168: `env_registry.py` NAMES variables — including the ones
            # belonging to sibling components, which this artifact deliberately
            # does not implement. Counting it as an implementation would let any
            # name satisfy this gate merely by being listed there, which is the
            # phantom flag this file exists to catch. Its own honesty gate is
            # `test_env_registry_matches_sources.py`.
