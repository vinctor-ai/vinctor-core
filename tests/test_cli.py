...
from vinctor_service.v1_http import handle_v1_delegated_enforce_http
...
def test_vinctor_demo_check_runs_smoke_flow() -> None:
    ...
    assert result["deny_probe_decision"] == "deny"
    ...
    # the /v1/auto-approval-rules POST). +1 again for the deny probe, which is
    # a second enforce call and therefore a second decision event.
    assert result["audit_event_count"] == 9


@pytest.mark.parametrize("demo_command", ["check", "block", "service"])
def test_vinctor_demo_refuses_the_connection_flags_it_would_discard(
    demo_command: str,
) -> None:
    # PKA-214: every `demo` subcommand accepted --endpoint/--agent-key/
    # --grant-ref, threw them away, ran its own throwaway service and reported
    # ok=true with exit 0. A dead endpoint, a nonexistent grant and a wrong key
    # all "passed". Refusing the flags is the only coherent alternative to
    # honouring them, because a demo must never write auto-approval rules into
    # somebody's real deployment.
    status, stdout, stderr = _run_failing(
        [
            "--json",
            "demo",
            demo_command,
    ...
    assert json.loads(stderr)["ok"] is False
    for flag in ("--endpoint", "--agent-key", "--grant-ref"):
        assert flag in stderr, stderr


def test_vinctor_demo_check_still_runs_with_ambient_environment_configuration(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # `vinctor local env` tells operators to export exactly these. An exported
    # value is ambient configuration, not an instruction to check that
    # deployment, so the demo must run rather than refuse -- only a flag typed
    # on the command line is a request the demo cannot honour.
    monkeypatch.setenv("VINCTOR_ENDPOINT", "http://127.0.0.1:9999")
    monkeypatch.setenv("VINCTOR_AGENT_KEY", "aak_TOTALLY_WRONG")
    monkeypatch.setenv("VINCTOR_GRANT_REF", "grt_does_not_exist")

    result = _run(["--json", "demo", "check"])

    assert result["ok"] is True
    assert "9999" not in result["throwaway_endpoint"]


def test_vinctor_demo_check_never_reports_an_endpoint_the_caller_can_reach() -> None:
    result = _run(["--json", "demo", "check"])

    # The old output printed `endpoint: http://127.0.0.1:<random>`, which an
    # operator reads as their own deployment answering.
    assert "endpoint" not in result
    assert result["target"] == "throwaway"
    assert result["throwaway_endpoint"].startswith("http://127.0.0.1:")


@pytest.mark.parametrize("demo_command", ["block", "service"])
def test_vinctor_demo_showcases_label_the_throwaway_endpoint(demo_command: str) -> None:
    result = _run(["--json", "demo", demo_command])
...
def test_vinctor_demo_check_text_never_presents_the_throwaway_as_your_service() -> None:
    text = _run_text(["demo", "check"])

    url_lines = [line for line in text.splitlines() if "http://127.0.0.1:" in line]
    assert url_lines, text
    # The URL and the word that disowns it must never drift into separate
    # lines: whoever reads the address must read what it is in the same breath.
    assert all("throwaway" in line for line in url_lines), text
    assert "does not check your deployment" in text.lower(), text


def test_vinctor_demo_check_fails_when_the_deny_probe_is_permitted(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # Seed an over-permissive deployment: the grant the check issues also
    # covers the deny-probe resource, so the negative control comes back
    # permit. Everything the old check looked at still succeeds here.
    monkeypatch.setattr(
        "vinctor_service.cli._DEMO_CHECK_GRANT_SCOPES",
        ("execute:ci/test", "delete:vinctor/demo/*"),
    )

    status, stdout, stderr = _run_failing(["--json", "demo", "check"])

    assert status == EXIT_SERVICE
    assert '"ok": true' not in stdout
    assert "deny probe" in stderr.lower(), stderr


def test_vinctor_demo_check_fails_when_the_granted_action_is_denied(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # Seed a deployment whose issued grant does not cover the action the check
    # enforces. The old code reported {"ok": true, "decision": "deny"}.
    monkeypatch.setattr(
        "vinctor_service.cli._DEMO_CHECK_GRANT_SCOPES", ("read:ci/logs",)
    )

    status, stdout, _ = _run_failing(["--json", "demo", "check"])

    assert status == EXIT_DENIED
    assert '"ok": true' not in stdout


def test_vinctor_demo_check_fails_when_the_throwaway_service_is_unreachable(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # A transport failure must fail the check. "Nothing came back" is not
    # evidence that anything was denied.
    def unreachable(config: LocalLaunchConfig, *, now: datetime | None = None):
        handle = prepare_local_service(config, now=now)
        handle.endpoint = f"http://127.0.0.1:{_closed_loopback_port()}"
        return handle

    monkeypatch.setattr("vinctor_service.cli.prepare_local_service", unreachable)
...
@pytest.mark.parametrize(
    ("status", "body"),
    [
        (200, {"decision": "permit", "reason": "permitted"}),
    ...
    ids=["permit", "unauthenticated", "server-error", "403-no-verdict", "empty", "200-deny"],
)
def test_demo_check_deny_probe_requires_a_verifiable_denial(
    status: int, body: dict[str, object]
) -> None:
    # The absence of a verdict is not a denial: an auth failure, a server
    # error, or a body with no decision must fail the probe, not pass it.
    with pytest.raises(CliError) as excinfo:
        _demo_check_deny_verdict(status, body)

    assert excinfo.value.code == EXIT_SERVICE


def test_demo_check_deny_probe_accepts_a_real_denial() -> None:
    reason = _demo_check_deny_verdict(
        403, {"decision": "deny", "reason": "action_denied"}
    )

    assert reason == "action_denied"
...
""".strip(),
    ...
    assert service_info["schema_versions"] == list(range(1, 19))
    assert service_info["schema_version"] == 18
...
def test_vinctor_cli_storage_backup_and_reset(tmp_path: Path) -> None:
    ...
    assert backup["schema_versions"] == list(range(1, 19))
    assert reset == {
        "db_path": str(db_path),
        "reset": True,
        "schema_versions": list(range(1, 19)),
    }
...
def test_vinctor_cli_service_info_reports_schema(tmp_path: Path) -> None:
    ...
    assert info["schema_version"] == 18
    assert info["schema_versions"] == list(range(1, 19))
...
def test_vinctor_cli_keys_create_pep_mints_a_usable_delegated_enforce_key(
    tmp_path: Path,
) -> None:
    # PKA-180: `vinctor-mcp-pep` authenticates to POST /v1/enforce/delegated with
    # an X-PEP-Key header, and nothing shipped could mint one. The property under
    # test is NOT that the CLI prints a `pep_`-shaped string — it is that the
    # minted secret actually authenticates that route. Assert it through the same
    # resolver the real server wires up (service_runtime/local_launcher pass
    # `resolve_pep_identity` as `pep_identity_resolver`), so a key created with
    # the wrong key_type, workspace, or pep_id fails here even though the CLI
    # still printed something.
    db_path = tmp_path / "vinctor.sqlite"
    _seed_storage_db(db_path)
    common = ["--json", "--db", str(db_path), "--workspace-id", "ws_demo"]

    created = _run([*common, "operator", "keys", "create", "pep", "--pep-id", "pep_ci"])
    assert created["raw_key"].startswith("pep_")
    assert created["key_type"] == "resource_server"
    assert created["agent_id"] == "pep_ci"
    assert created["workspace_id"] == "ws_demo"

    response = _delegated_enforce_http(db_path, created["raw_key"])
    assert response.status_code == 200
    assert response.body["decision"] == "permit"

    # An unknown key still fails closed, so the permit above cannot be an
    # "any key authenticates" regression.
    unauthenticated = _delegated_enforce_http(db_path, f"pep_{'z' * 32}")
    assert unauthenticated.status_code == 401
    assert unauthenticated.body["error"] == "authentication_required"

    conn = connect_sqlite(db_path)
    try:
        service = SQLiteV1Service(conn, initialize_schema=False)
        control = [e for e in service.audit_writer.list_all() if e.event_class == "control"]
        created_events = [e for e in control if e.event_type == "key_created"]
        assert len(created_events) == 1
        assert created_events[0].resource == f"key/resource_server/{created['key_id']}"
        assert created_events[0].agent_id == "pep_ci"
        assert created_events[0].enforcing_principal == "workspace:ws_demo"
        assert service.audit_writer.verify_chain().ok is True
    finally:
        conn.close()


def test_vinctor_cli_keys_create_pep_does_not_revoke_existing_pep_keys(
    tmp_path: Path,
) -> None:
    # `create` is not `rotate`: minting a second PEP key must leave the first one
    # working, or adding a PEP silently locks the deployed one out.
    db_path = tmp_path / "vinctor.sqlite"
    _seed_storage_db(db_path)
    common = ["--json", "--db", str(db_path), "--workspace-id", "ws_demo"]

    first = _run([*common, "operator", "keys", "create", "pep", "--pep-id", "pep_ci"])
    second = _run([*common, "operator", "keys", "create", "pep", "--pep-id", "pep_ci"])

    assert first["revoked_key_ids"] == []
    assert second["revoked_key_ids"] == []
    assert first["raw_key"] != second["raw_key"]
    assert _delegated_enforce_http(db_path, first["raw_key"]).status_code == 200
    assert _delegated_enforce_http(db_path, second["raw_key"]).status_code == 200

    # A rotation still supersedes both, so `create` did not weaken revocation.
    _run([*common, "operator", "keys", "rotate", "pep", "--pep-id", "pep_ci"])
    assert _delegated_enforce_http(db_path, first["raw_key"]).status_code == 401
    assert _delegated_enforce_http(db_path, second["raw_key"]).status_code == 401


def test_vinctor_cli_keys_create_pep_text_output_shows_the_secret_once(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "vinctor.sqlite"
    _seed_storage_db(db_path)
    common = ["--db", str(db_path), "--workspace-id", "ws_demo"]

    text = _run_text([*common, "operator", "keys", "create", "pep", "--pep-id", "pep_ci"])
    raw_key = next(
        line.split("=", 1)[1] for line in text.splitlines() if line.startswith("raw_key=")
    )
    assert raw_key.startswith("pep_")
    assert "cannot be recovered" in text

    # SQLite stores only the digest: the secret is unrecoverable afterwards.
    listed = _run(["--json", *common, "operator", "keys", "list"])
    assert raw_key not in json.dumps(listed)
    assert "raw_key" not in json.dumps(listed)
...
def test_vinctor_cli_require_boundary_show_reflects_workspace_rollout(tmp_path) -> None:
    # The documented rollout: turn the mandate on workspace-wide, then confirm
    # it for an agent that has no override of its own. `show <agent>` must not
    # report that agent as exempt — it is the command an operator uses to check
    # whether the rollout landed.
    db_path = tmp_path / "vinctor.sqlite"
    common = ["--json", "--db", str(db_path), "--workspace-id", "ws_demo"]
    _run([*common, "operator", "require-boundary", "enable", "--workspace"])
    shown = _run([*common, "operator", "require-boundary", "show", "agent_runner"])
    assert shown["require_boundary"] is True

    # And the reverse: disabling workspace-wide must not leave `show` claiming
    # the mandate is still on for an agent without its own override.
    _run([*common, "operator", "require-boundary", "disable", "--workspace"])
    shown_off = _run([*common, "operator", "require-boundary", "show", "agent_runner"])
    assert shown_off["require_boundary"] is False
...
def _delegated_enforce_http(db_path: Path, pep_key: str) -> V1HttpResponse:
    """Call POST /v1/enforce/delegated the way the running server does.

    The resolver wiring mirrors service_runtime/local_launcher exactly, so this
    exercises the real X-PEP-Key authentication path rather than a test double.
    """
    conn = connect_sqlite(db_path)
    try:
        ...
        return handle_v1_delegated_enforce_http(
            headers={
                "X-PEP-Key": pep_key,
                "X-Vinctor-Boundary-Id": "bnd_cli_test",
            },
            body={
                "workspace_id": "ws_demo",
                "agent_id": "agent_runner",
            ...
            pep_identity_resolver=lambda raw_key, used_at: repository.resolve_pep_identity(
                raw_key, now=used_at
            ),
            service=SQLiteV1Service(conn, initialize_schema=False),
            now=NOW,
...
def _run_failing(argv: list[str]) -> tuple[int, str, str]:
    stdout = StringIO()
    stderr = StringIO()
    status = run_vinctor(argv, stdout=stdout, stderr=stderr)
...
@pytest.mark.parametrize(
    "command",
    [
        ["operator", "bounds", "set", "--scope", "read:repo/docs/*"],
        ["operator", "bounds", "show"],
        ["operator", "keys", "list"],
        ["operator", "audit", "list"],
        ["operator", "require-boundary", "show"],
        ["operator", "storage", "migrate"],
    ],
)
def test_operator_db_only_commands_name_the_flag_and_explain_the_split(
    command: list[str], monkeypatch: pytest.MonkeyPatch
) -> None:
    """A missing --db is a split-control-plane surprise, not a typo (PKA-254).

    Almost every `operator` resource runs against the local SQLite file while a
    handful of neighbours go over HTTP, so the error has to name the flag, name
    the env var, and say why this command is local-only.
    """
    monkeypatch.delenv("VINCTOR_DB", raising=False)
    stdout = StringIO()
    stderr = StringIO()

    status = run_vinctor(command, stdout=stdout, stderr=stderr)

    assert status == EXIT_USAGE
    message = stderr.getvalue()
    assert "--db" in message
    assert "VINCTOR_DB" in message
    assert "--endpoint" in message
    assert "no remote operator route" in message


def test_operator_audit_reason_filter_finds_the_reason_it_printed(tmp_path: Path) -> None:
    """`--reason` must match the code the list output just showed (PKA-254).

    Decision denies carry the cause in `reason` and no `reason_code`, so a
    reason_code-only filter returned nothing for a code the operator had read
    ...
    try:
        ...
        conn.close()
    # A decision deny: the cause is in `reason`, and `reason_code` is unset.
    assert denied.reason == "boundary_required"

    listed = _run(
        ["--json", "--db", str(db_path), "operator", "audit", "list", "--reason",
         "boundary_required"]
    )

    assert [event["reason"] for event in listed["audit_events"]] == ["boundary_required"]
    assert listed["audit_events"][0]["reason_code"] is None
