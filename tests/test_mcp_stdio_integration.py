...
def test_real_stdio_grant_report_pages_complete_audit_history() -> None:
    ...
    with _running_service() as port:
        for index in range(125):
            _enforce(port, "write", f"repo/demo/item-{index}")
        report = asyncio.run(_with_session(port, body))

    assert len(report["usage"]) == 125
    assert report["usage"][0]["resource"] == "repo/demo/item-0"
    assert report["usage"][-1]["resource"] == "repo/demo/item-124"
