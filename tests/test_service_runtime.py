...
def test_service_runtime_closes_background_audit_export(
    ...
    export = Export()
    monkeypatch.setattr(
        "vinctor_service.sqlite.audit_sinks_from_env",
        lambda _env: AuditSinks(anchor=NullAnchor(), export=export),
    )
    handle = prepare_service_runtime(
        ServiceRuntimeConfig(sqlite_db_path=tmp_path / "vinctor.sqlite", port=0),
        clock=lambda: NOW,
...
def test_service_runtime_wires_delegated_pep_enforce(tmp_path: Path) -> None:
    ...
    assert bootstrap.boundary is not None
    boundary_id = bootstrap.boundary.boundary_id
    bootstrap.close()
    ...
    try:
        with running_runtime(handle):
            permit_status, permit, _ = request_json(
                ...
                headers={
                    "X-PEP-Key": "pep_runner",
                    "X-Vinctor-Boundary-Id": boundary_id,
                },
                body={
                    "workspace_id": "ws_demo",
                    "agent_id": "agent_runner",
