...
def _audit_row_count(db_path: Path) -> int:
    """Count audit rows on an INDEPENDENT connection.

    Counting on the writer's own connection sees uncommitted rows, so it cannot
    tell "durably audited" from "written inside a transaction that will never
    commit" — which is how a permit-with-no-durable-audit-row shipped (PKA-224).
    Only a separate connection answers the question this test is asking.
    """
...
def test_audit_write_failure_returns_503_with_no_decision_field(
    ...
    try:
        ...
        rows_seeded = _audit_row_count(db_path)
        ...
        rows_before = _audit_row_count(db_path)
        ...
        assert _audit_row_count(db_path) == rows_before
