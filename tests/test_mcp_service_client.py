...
def test_client_requests_a_bounded_audit_page() -> None:
    client, conn = make_client(
        FakeResponse(
            200,
            {
                "audit_events": [],
                "page": {
                    "snapshot_event_id": "evt_snapshot",
    ...
    body = client.list_audit_events(
        ...
        paginate=True,
        snapshot_event_id="evt_snapshot",
        before_event_id="evt_before",
    )

    assert body["page"]["snapshot_event_id"] == "evt_snapshot"
    parsed = urlsplit(conn.requests[0]["path"])
    assert parse_qs(parsed.query) == {
        "before_event_id": ["evt_before"],
        "grant_ref": ["grt_main"],
        "limit": ["100"],
