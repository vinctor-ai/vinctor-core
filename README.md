...
```bash
docker pull ghcr.io/vinctor-ai/vinctor-core:0.7.0     # or :latest
docker run --rm ghcr.io/vinctor-ai/vinctor-core:0.7.0 vinctor --help
```
...
The launcher prints copy-pasteable exports. **They belong in two different
shells.** The runtime that calls the boundary gets:
...
The workspace key is an **operator** credential and stays in an operator-only
shell the agent cannot read:

```bash
export VINCTOR_WORKSPACE_KEY="wsk_..."   # operator shell ONLY
```

That split is the whole point of the model, not a hardening tip. An agent whose
environment contains `VINCTOR_WORKSPACE_KEY` can approve its own grant requests
and widen its own issuable scope bounds — it can grant itself the authority
Vinctor exists to withhold. See
[Renewing a grant](docs/cli-reference.md#renewing-a-grant) for the procedure
that keeps the two apart when a grant expires.

Keep the raw keys outside the repository. SQLite stores only key hashes and
metadata, not raw workspace or agent keys.

`agent` commands read `VINCTOR_ENDPOINT`/`VINCTOR_AGENT_KEY` from the
environment and exit with `error: agent key is required` in a shell that does
not have them; `operator` commands read `VINCTOR_WORKSPACE_KEY` and are meant to
be run from the operator shell.
...
Agents can request grants, then operators decide them. The two halves run in the
two shells above — that separation is what makes the decision an authorization
rather than a formality:

```bash
# agent shell (VINCTOR_AGENT_KEY)
...
```bash
# operator shell (VINCTOR_WORKSPACE_KEY)
...
- **It is settable only through `--db` today, and `--db` is SQLite.**
  `operator require-boundary` opens a SQLite file directly; there is no
  Postgres code path, no HTTP route and no MCP tool for it, and `--endpoint` is
  ignored. An operator without filesystem access to a SQLite deployment — or
  running Postgres at all — cannot turn it on through any supported interface;
  on Postgres the only route is direct SQL, which leaves no control-plane audit
  record. (`operator policy apply` can enable it additively from a policy file,
  but it is `--db`/SQLite-only for the same reason.)
...
**Upgrading an existing deployment?** A migration never turns the mandate on —
the column default your database was created with is what agents without an
override get, and `CREATE TABLE IF NOT EXISTS` never rewrites it. See
[Upgrading to 0.6.0: turning on the boundary mandate](docs/upgrade-0.6-require-boundary.md)
for how to read what your database does today and how to roll the mandate out
without denying working callers. It also covers what a **Postgres** operator can
and cannot do here, which is the sharper limit.
...
`VINCTOR_WORKSPACE_KEY` belongs in an operator-only shell, not in the runtime
that calls the boundary — see [Renewing a grant](docs/cli-reference.md#renewing-a-grant)
for the procedure that keeps it there when a grant expires.
...
A single smoke check of the packaged build ships with the package itself:

```bash
vinctor --json demo check
```

It runs a granted action that must be permitted and a deny probe that must be
denied, against a throwaway local service it starts and destroys. It says
nothing about a deployment, and refuses connection flags such as `--endpoint`
rather than discarding them; use `vinctor agent enforce` against a real
deployment.
