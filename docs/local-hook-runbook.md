...
Run this from an **operator shell**. Bootstrap legitimately holds both keys
because it creates both; the hook runtime in step 2 gets only the agent-side
values, and renewing an expired grant stays on this side of the line — see
[Renewing a grant](cli-reference.md#renewing-a-grant).
...
Fresh 0.6 databases require `VINCTOR_BOUNDARY_ID` for enforcement. `vinctor
local start` provisions and exports it automatically. Upgraded databases keep
their existing `require_boundary` setting, so the header is optional only where
an operator has retained or explicitly configured that legacy behavior. The
value also records which configured runtime boundary originated the audit row.
