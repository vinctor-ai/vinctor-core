...
▸ 3 decisions · 3 audit records · nothing out-of-scope ran.
  Vinctor authorizes configured, mediated calls that reach its boundary; it is not a sandbox.
```
...
- The footer states "authorizes configured, mediated calls that reach its
  boundary; it is not a sandbox", consistent with existing repo labeling. The
  coverage qualifier is load-bearing: Vinctor decides the calls that are
  configured to reach it, not every call the agent can make.
