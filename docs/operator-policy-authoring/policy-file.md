...
| `max_ttl` or `max_ttl_seconds` | No | Maximum TTL the service may issue to this agent. Absent preserves any stored cap; an explicit `null` clears it; a value sets it. |
...
A bounds entry that is silent on `max_ttl` never touches the stored issuance
cap: re-applying a policy cannot silently widen an agent's issuable authority.
Removing a cap requires an explicit `max_ttl: null`. Export includes each
agent's cap (as `max_ttl`) when one is set, so export -> edit -> apply
round-trips the cap exactly.
