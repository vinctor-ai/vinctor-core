...
paths, exceptions, and internal configuration.

`GET /readyz`

Auth: none

`/healthz` is a liveness check on **every** backend: it performs no database
operation, takes no lock, and answers `200` for as long as the process can
serve. It is not a readiness gate — a store outage does not change it.

`/readyz` is the traffic-readiness check: it probes the durable store and
returns `200` with `status: ready`, or `503` with `status: unavailable`. The
probe runs under an explicit deadline (`VINCTOR_READINESS_PROBE_TIMEOUT_SECONDS`,
default 2s), so a hung backend socket makes `/readyz` answer `unavailable` at
the bound rather than blocking with the socket. The failure response exposes no
DSN, path, exception, or database error.

Response *timing* is not covered by that guarantee and is not treated as
confidential: a hung store answers at the bound while a refused connection
answers immediately, so latency distinguishes the two. The endpoint is
unauthenticated and reports coarse reachability by design — the bodies and
headers disclose nothing further.
...
| `X-Vinctor-Boundary-Id` | `POST /v1/enforce`, `POST /v1/enforce/delegated` | Runtime boundary identity. Required when the workspace has `require_boundary` enabled (the default for fresh databases); otherwise optional audit context. |
| `Idempotency-Key` | Exactly seven mutation routes listed below | Optional caller-generated retry identity. It is never returned, logged, audited, or stored raw. |
...
`Idempotency-Key` is supported on exactly these seven routes:
...
The caller generates a fresh key for one effective mutation. UUIDv4 or an independently
generated value with at least 128 bits of entropy is recommended. The accepted wire
grammar is one ASCII header value matching `[A-Za-z0-9._~-]{1,128}`. Reuse the same key
only for a byte-equivalent effective request in the same authenticated
workspace/principal/operation scope.

The header is optional. Omitting it preserves the pre-existing unkeyed mutation path,
including when the idempotency keyring is not configured. Supplying it opts into
fail-closed encrypted replay and requires the operator-injected keyring.
Authentication and JSON parsing run before retry-key validation.

Responses specific to this contract are deliberately coarse:

| Status | Error/reason | Meaning |
| --- | --- | --- |
| `400` | `invalid_idempotency_key` | Missing uniqueness on the wire (duplicate or combined fields), invalid ASCII grammar, or excessive length. The request is not cached. |
| `409` | `idempotency_key_conflict` | The scoped key already names a different effective request. No mutation or new reservation occurs. |
| `503` | `idempotency_unavailable` | The configured keyring, historical decryption key, registry commitment, ciphertext, or durable result authority cannot be verified. The mutation is not retried. |

Authenticated cacheable terminal outcomes are replayed with the original status,
content type, body bytes, and content length. The raw retry key, bearer token, serialized
plaintext response, PoP material, AES key, and database DSN never enter idempotency
tables, logs, errors, or audit rows. PKA-19's separate authoritative
`subject_tokens.pop_secret` plaintext debt is unchanged.
...
### Operator-only deny reason codes

These never appear in an agent-facing response. They are the `reason_code` on
the operator audit event, readable with a workspace or auditor key
(`vinctor operator audit list --reason <code>`, `GET /v1/audit-events?reason_code=`).
The split exists because a fine-grained deny reason in a response is a
policy-probing side channel, while an operator with no cause cannot tell whether
to widen a grant, edit a rule, or stop.

Behind the single generic `403 forbidden` + `subject token is not valid`
response — the caller never learns which check it tripped, but the remedies
differ, so the audit records the cause:

| `reason_code` | Meaning | Operator's move |
| --- | --- | --- |
| `subject_token_not_found` | The presented token matches no record. | The agent must re-mint. |
| `subject_token_expired` | The token is past its expiry. | The agent must re-mint. |
| `subject_token_revoked` | The token was explicitly revoked. | Usually working as intended. |
| `subject_token_mismatch` | The token is live but disagrees with the request, the grant, the audience, or its own action/resource binding. | Fix the PEP configuration or the mint parameters. |
| `pop_proof_invalid` | Proof-of-possession verification failed (missing, stale, replayed, or wrong secret). | Check clock skew and the shared secret. |
