...
**Since this run (PKA-117):** `/readyz` now hands its store probe to a
persistent single-flight worker under a 2s deadline instead of running it on
the request thread. The handoff is two `Event` operations and a lock, not a
thread spawn, so it is off the measured critical path; a controlled 8-client
A/B on the same machine put `/readyz` at 3,458 req/s with the bound versus
3,398 without, and `/healthz` at 4,275 versus 4,276 — no difference outside
run-to-run noise. (That A/B used an in-process harness, so its absolute numbers
are not comparable with the table above; only the with/without delta is.) The
`/healthz` column also now holds on the Postgres backend, which previously ran
the store probe on the liveness path.
...
and takes no pooled connection on any backend (`DATABASE_FREE_PATHS`,
