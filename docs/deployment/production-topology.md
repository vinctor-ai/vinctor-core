...
| --- | --- | --- | --- |
| `/healthz` | GET | none | Liveness. `200 {"status":"ok","service":"vinctor-service","mode":...}`. Performs no database operation on any backend and answers `200` for as long as the process can serve; exempt from the rate limiter (GET only). |
| `/readyz` | GET | none | Readiness. `200 {"status":"ready",...}` when the store answers `SELECT 1`; `503 {"status":"unavailable",...}` otherwise (fails closed, leaks no connection detail). The probe is bounded (`VINCTOR_READINESS_PROBE_TIMEOUT_SECONDS`, default 2s) and single-flight. |
...
| `VINCTOR_AUDIT_SINK_REQUIRED` | off | `true` | Refuses startup unless at least one valid `VINCTOR_AUDIT_ANCHOR` or `VINCTOR_AUDIT_EXPORT` sink is configured. |
...
Liveness and readiness answer different questions and must be wired to
different endpoints:

- `/healthz` — **liveness only.** "Can this process serve at all?" It performs
  no database operation, so it stays `200` throughout a store outage. Never use
  it as a readiness or traffic gate.
- `/readyz` — **readiness.** "Can this instance safely take enforcement
  traffic?" It probes the durable store under a bounded deadline (default 2s)
  and returns `503` when the store does not answer.
...
- Kubernetes: `/healthz` on `livenessProbe`, `/readyz` on `readinessProbe`.
  Because `/readyz` fails closed on backend loss, a store outage drains the pod
  from rotation without killing it — and because `/healthz` runs no store
  probe, the outage cannot restart the whole fleet at once.
...
readinessProbe:
  ...
  periodSeconds: 10
  # Set above the endpoint's own probe bound (default 2s) so a slow store
  # reports 503 "unavailable" rather than timing out here indistinguishably.
  timeoutSeconds: 5
  failureThreshold: 2
```

**Size the probe bound against your store, not against the default.** On
Postgres the readiness probe takes the serialized connection's lock — the same
lock every enforce request contends for — and runs the idempotency-readiness
queries (including a `SELECT DISTINCT cipher_key_version FROM
idempotency_results`, whose cost grows with the table) as well as `SELECT 1`.
Under heavy enforce load or with a large `idempotency_results` table, a healthy
store can exceed the 2s default, and readiness cannot tell "slow" from "dead":
the busiest instance would leave rotation and shift its load onto its peers.
Raise `VINCTOR_READINESS_PROBE_TIMEOUT_SECONDS` above the p99 connection-lock
wait plus probe time, and keep `timeoutSeconds` and the container healthcheck
timeout above that. `failureThreshold: 2` with `periodSeconds: 10` drains a pod
about 20s after it starts failing, so a bound set too low costs availability
quickly.

`/readyz` is rate limited like every other route (only `/healthz` is exempt),
so keep `VINCTOR_RATE_LIMIT_PER_MINUTE` above your probe rate — see
[cli-reference.md](../cli-reference.md). Readiness probes from an orchestrator
share one rate-limit source. Note that the limiter is opt-in and off by
default, and readiness results are not cached: against a healthy store each
`/readyz` request costs one backend round-trip. Single-flight caps how many run
