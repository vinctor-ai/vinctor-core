...
Upgrades are explicit and operator-controlled. SQLite migration 18 adds the
authoritative encrypted idempotency-result schema and therefore requires a
maintenance window when upgrading from schema 17:

1. Stop every service replica, worker, CLI loop, and other database writer.
2. Take a complete pre-migration backup and verify that the snapshot can be
   opened or restored in a separate drill. Retain an out-of-band checksum.
3. Install the target package and run the migration once while writers remain
   stopped.
4. Confirm the reported schema sequence is exactly `1` through `18`.
5. Start only binaries at the target version. Do not mix old and new binaries.

```bash
# 1. Enter the maintenance window before taking the rollback snapshot.
sudo systemctl stop vinctor.service

# 2. Snapshot the quiesced database (see SQLite Backup And Restore).
sudo -u vinctor /opt/vinctor/.venv/bin/vinctor \
  --db /var/lib/vinctor/vinctor.sqlite \
  operator storage backup --output /var/backups/vinctor/pre-upgrade.sqlite

# 3. Install the target package version.
sudo /opt/vinctor/.venv/bin/pip install --upgrade "vinctor-core==<target-version>"

# 4. Apply and confirm the on-disk schema (idempotent and data-preserving).
sudo -u vinctor /opt/vinctor/.venv/bin/vinctor \
  --db /var/lib/vinctor/vinctor.sqlite operator storage migrate

# 5. Start the target binary only after it reports schema versions 1 through 18.
sudo systemctl start vinctor.service
```

If rollout must be reversed after migration 18, stop every writer and restore the
entire verified schema-17 snapshot. Do not delete migration row 18, drop the new
tables, or edit their contents to make an old binary start; that is not a
supported downgrade and can invalidate encrypted replay state or the audit chain.

## Encrypted Idempotency Key Lifecycle

Key material is supplied only by the deployment's external secret system through
`VINCTOR_IDEMPOTENCY_KEYRING_JSON` and
`VINCTOR_IDEMPOTENCY_ACTIVE_KEY_VERSION`. The service and operator CLI do not generate,
accept as arguments, switch, or print AES keys. Keep the JSON itself out of shell
history, logs, tickets, and command output.

The lifecycle CLI has exactly four subcommands:

```bash
vinctor --db /var/lib/vinctor/vinctor.sqlite operator idempotency status
vinctor --db /var/lib/vinctor/vinctor.sqlite operator idempotency write-disable \
  --version <old-version> --reason rotation
vinctor --db /var/lib/vinctor/vinctor.sqlite operator idempotency drain-complete \
  --version <old-version> --confirm-no-active-writers
vinctor --db /var/lib/vinctor/vinctor.sqlite operator idempotency retire \
  --version <old-version> --confirm-removal-window
```

`status` prints only immutable labels, a one-way commitment identifier, lifetime
reservation counts, lifecycle reasons and epochs, required historical versions, and
the locally active version. It never prints the keyring JSON, raw commitments, AES
keys, ciphertext, database credentials, or internal retirement counts. Every mutating
command requires one explicit historical label and rejects the locally active label.

Rotate one version as follows:

1. Generate a new 32-byte AES key in the approved secret system. Update the injected
   keyring to contain both labels and set only the new label active.
2. Roll the new configuration to every current binary. Require `/readyz` to return
   `200` everywhere before continuing. A missing historical key, commitment mismatch,
   unknown unexpired result version, or disabled/retired active label fails readiness.
3. Run `status`, then `write-disable --version <old> --reason rotation`. This blocks
   every later reservation under the old label. The command is idempotent.
4. Drain and stop every process that could still write with the old configuration.
   Confirm process termination through the deployment control plane. Participating
   keyed services automatically hold the backend writer-attestation lock for their
   active key version for their full lifetime, including every SQLite pool generation
   and PostgreSQL replacement connection. `drain-complete` and `retire` take the
   exclusive lock for the historical target, so an old-version writer blocks them
   while a replacement-only writer does not. Only then supply
   `--confirm-no-active-writers`.
5. Wait until database time is at least `86,400 + 300` seconds after the recorded drain
   epoch. Host clocks and a command-line timestamp are not accepted. Expired-result GC
   deletes only rows with `expires_at_epoch <= database_now`, in batches no larger than
   100; nonce reservations, lifetime counts, and version rows are never reclaimed.
6. Require `status` to report a healthy decrypt-capable active replacement. Run
   `retire --confirm-removal-window`; it checks the authoritative unexpired-result
   count internally and refuses while any old-version result remains.
7. Verify the retired tombstone and final slot count with `status`, restart a process,
   and require `/readyz` to remain `200`. Only then remove the old key bytes from the
   externally injected keyring.

Premature commands fail closed without advancing the barrier. On a lost or ambiguous
database acknowledgement, treat the command as failed until a fresh authoritative
`status` confirms the durable state; never edit lifecycle columns, decrement reservation
counts, or delete tombstones manually. There are intentionally no `generate`, `rotate`,
`activate`, or `switch` lifecycle commands.
