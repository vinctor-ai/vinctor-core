...
- **Over HTTP, authenticated by a key** (needs `--endpoint` and the key; the key
  ...
  - `operator requests …` and `operator rules …` → the **workspace key**
    ...
    never in the shell the agent runs in, including when a grant needs renewing
    (see [Renewing a grant](#renewing-a-grant)).
...
`{enable, disable, show}` shape. `require_subject_token` and `require_pop`
default off. On a database created by 0.6.0, `require_boundary` defaults on;
upgraded databases retain their earlier default and stored overrides. A bare
invocation targets the agent from `--agent-id` (or a positional
`target_agent_id`); pass `--workspace` to set the **workspace default** instead
(the per-agent value overrides it). `--workspace` cannot be combined with an
agent id.
...
A grant is the scoped, time-boxed permission set a mediated call is checked
against.
...
Grants expire, and renewal needs workspace/operator authority. **Do not get that
authority by exporting `VINCTOR_WORKSPACE_KEY` into the shell the agent runs
in.** An agent that can read the workspace key can approve its own grant
requests and widen its own issuable scope bounds — it can grant itself the
authority Vinctor exists to withhold. Privilege separation here is not a
property of the key; it is a property of which process can read it.

Renewal therefore runs in an **operator-only shell**, and the only thing that
crosses back to the agent is the new `grant_ref` — which is not a credential and
confers nothing on its own.

```bash
# operator-only shell: has VINCTOR_WORKSPACE_KEY, must NOT have VINCTOR_AGENT_KEY
...
It prints exactly one line, and no key appears in it:

```text
export VINCTOR_GRANT_REF="grt_…"
```

Set that in the agent's shell (or its process manager / hook config). Nothing
else about the agent's environment changes.

The script refuses to run when `VINCTOR_AGENT_KEY` is set in its own
environment: both keys in one environment means the separation has already
collapsed, so that is a hard error rather than a warning. It reads the workspace
key from `VINCTOR_WORKSPACE_KEY` or `--key-file`, never from a command-line
argument — argv is readable by every process on the host through `ps`.

`tools/` is a repository directory, not part of the installed wheel; fetch the
script from a checkout. If you would rather not run it, the shape is all that
matters: `POST /v1/grants` with `X-Workspace-Key`, issued from somewhere the
agent cannot reach, returning a `grant_ref` you hand over.

Renewal issues a *new* grant. The agent keeps working under the old one until
its `expires_at`, so a renewal that lands before expiry is seamless; revoke the
old ref with `operator grants revoke` if it should stop being usable
immediately.
...
vinctor demo check                   # self-check of the packaged build
vinctor demo service --scenario …    # scripted demonstration service
```

> [!IMPORTANT]
> `demo` commands never contact a configured deployment. Each one starts a
> throwaway local service, exercises it, and destroys it. They **refuse**
> `--endpoint`, `--agent-key`, `--grant-ref` and the other connection flags
> rather than discard them, and they report the throwaway's address as
> `throwaway_endpoint`, never as `endpoint`.
>
> `demo check` proves the packaged build enforces a grant on this machine — it
> runs a granted action that must be permitted **and** a deny probe that must be
> denied. It proves nothing about your deployment. To exercise that, use
> `vinctor agent enforce` against it.
