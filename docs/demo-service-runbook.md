...
For a single sanity check of the packaged build:
...
request, auto-approves it, enforces the granted action (which must be
permitted), enforces a fresh out-of-grant action (which must be denied), and
verifies both decisions were written to the audit log.

It does **not** check a deployment. It starts its own throwaway service, so it
refuses `--endpoint`, `--agent-key`, `--grant-ref` and the other connection
flags instead of discarding them. To exercise a real deployment, run
