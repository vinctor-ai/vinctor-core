...
Superseded in part. Accepted 2026-07-18; amended 2026-07-28.

The original decision below remains the historical rationale for
`require_subject_token` and `require_pop`. Release 0.6.0 changes only
`require_boundary`: new databases default it on, while existing databases keep
their prior schema default and stored overrides. The owner accepted that
fresh-install compatibility break after PKA-179 showed that a caller-controlled
header otherwise bypasses a disabled boundary.
